...
c                               Be sure SLFTST sets all print flags off.
   ...
   10 CONTINUE
c++csz
c      IF( .NOT.PASS1 .AND. LEN( HEADER ).NE.0 ) WRITE( *,
c     &    FMT = '(//,1X,120(''*''),/,25X,A,/,1X,A,/,1X,120(''*''))' )
c     &    'Discrete Ordinates Radiative Transfer Program, Version 1.1',
c     &    HEADER
c--csz
...
c   Called by- DISORT
c   Calls- LEPOLY, ZEROIT, SOLEIG, TERPEV, SETMTX, SOLVE1, ALTRIN,
c          SPALTR, PRALTR
c +-------------------------------------------------------------------+
...
c     .. Local Scalars ..
      ...
      INTEGER   IHOM, IQ, IU, L, LC, MAZIM, NCOL, NCUT, NHOM
      REAL      DELM0, FISOT, SGN, SPHALB, SPHTRN
c     ..
c     .. External Subroutines ..

      EXTERNAL  ALTRIN, LEPOLY, PRALTR, SETMTX, SOLEIG, SOLVE1, SPALTR,
     &          TERPEV, ZEROIT
c     ..
c     .. Intrinsic Functions ..
...
c                         boundary and layer interface conditions
      ...
      CALL SETMTX( BDR, CBAND, CMU, CWT, DELM0, DTAUCP, GC, KK, LAMBER,
     &             LYRCUT, MI, MI9M2, MXCMU, NCOL, NCUT, NNLYRI, NN,
     &             NSTR, TAUCPR, WK )
...
c                                      positive UMU inst. of negative
   ...
   40       CONTINUE
c                                  ** Get spherical albedo and, for one
c                                     layer, spherical transmissivity

            IF( ALBEDO.GT.0.0 ) CALL SPALTR( CMU, CWT, GC, KK, LL,
     &                               MXCMU, NLYR, NN, NSTR, TAUCPR,
     &                               SPHALB, SPHTRN )
...
c                                sources, remainder of B array is zero
   ...
   10    CONTINUE
         ...
         IF( 1.0 + RCOND.EQ.1.0 )
     &       CALL ERRMSG('SOLVE1--SGBCO says matrix near singular',
     &                   .FALSE.)
...
c       ZJ     :  Solution vector Z-sub-zero after solving
c                    Eq. SS(19)
c       (remainder are DISORT input variables)
...
c csz++ Library routines stored in machine.F in disort1.f

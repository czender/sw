...
c     Print some useful diagnostics if array bounds need to be reset
      if (nmom .gt. mxcmu) stop 'Increase mxcmu in disort2.f'
      if(nlyr .gt. mxcly) stop 'Increase mxcly, mxulv in disort2.f'
c      IF( PASS1 .AND. LEN( HEADER ).NE.0 )
...
c dwf++
c remove DREF check, computationally expensive, DWF
c           DO 70 IRMU = 0, 100

c              RMU  = IRMU*0.01
c              FLXALB = DREF( WVNMLO, WVNMHI, RMU )

c              IF( FLXALB.LT.0.0 .OR. FLXALB.GT.1.0 )
c    &             INPERR = WRTBAD( 'FUNCTION BDREF' )
...
c csz++ Some library routines stored in machine.F in disort1.f are 
c     for lack of a better place, stored here in disort2.f

...
c       ** Specify users bottom reflectivity and emissivity properties
        IF( .NOT.LYRCUT ) THEN
          CALL SURFAC( ALBEDO, FBEAM, LAMBER, MI, MAZIM,
...
c         ** Version 3: update SOLEIG argument: EIGEN_MAT
          CALL SOLEIG( AMB, APB, EIGEN_MAT, CMU, CWT, GL( 0,LC ), MI,
...
c     .. Array Arguments ..
      ...
      REAL      AMB( MI, MI ), APB( MI, MI ), ARRAY( NN, NN ),
...
c                      ** Find (real) eigenvalues and eigenvectors
      ...
      CALL ASYMTX( ARRAY, EVECC, EVAL, NN, NN, MXCMU, IER, WKD, AAD,
     &             EVECCD, EVALD )
...
c     .. Array Arguments ..
     ...
     &          ZZ( MXCMU, * )
      ...
      DOUBLE PRECISION LEFT_MAT( MI9M2, NNLYRI ), RIGHT_COL(NNLYRI)
c     ..
c     .. Local Scalars ..
...
c                     ** (CBAND is in LAPACK packed format)
      ...
      CALL SGBTRF( NCOL, NCOL, NCD, NCD, CBAND, MI9M2, IPVT, INFO )
...
c                   ** decomposed.  Solution is returned in B.
      ...
      CALL SGBTRS( 'N', NCOL, NCD, NCD, 1, CBAND, MI9M2, IPVT,
     &              B, NNLYRI, INFO   )
...
c
c      RCOND  = 0.0
c      CALL SGBCO( CBAND, MI9M2, NCOL, NCD, NCD, IPVT, RCOND, Z )
c      IF( 1.0 + RCOND.EQ.1.0 )
c     &   CALL ERRMSG('SOLVE0--SGBCO says matrix near singular',.FALSE.)
c      CALL SGBSL( CBAND, MI9M2, NCOL, NCD, NCD, IPVT, B, 0 )
...
c     csz++ This warning is often triggered by low vertical resolution atmospheres
c     I've changed temperature jump for warnings from DISORT-default 10 K to 20 K
c     because stratospheric temperature jumps in 18 level atmospheres are just under 20 K
c     Feel free to change this if necessary

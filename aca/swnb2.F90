...
program swnb2
  ...
  allocate(flx_nst_up(levp_nbr),stat=rcd)
  if(rcd /= 0) stop "allocate() failed for flx_nst_up"
  ! Array dimensions: plr
...
120     if ((wvl_ctr(bnd_idx)<wvl_min_O3(bnd_idx_tmp_O3)).or. &
     ...
     if (slr_zen_ngl_cos > 0.0) then ! daytime
        ...
        rfl_drc_spc_snw(:,:)=0.0
        rfl_dff_spc_snw(:,:)=0.0
!        goto 173 ! Jump to just after DISORT
     endif ! night-time
...
!     write (6,'(a,i2,a,es8.1)') ' flurt flx_bb_abs(',lev_idx,') =  ',flx_bb_abs(lev_idx)
  ...
  if (mode_std) then
     ...
     rcd=nf90_wrp(nf90_def_var(nc_id,'flx_nst_up',nf90_float,levp_dmn_id,flx_nst_upw_id),sbr_nm//': dv flx_nst_up')
     ...
     rcd=nf90_wrp(nf90_put_var(nc_id,flx_nst_upw_id,flx_nst_up),sbr_nm//': pv flx_nst_up in '//__FILE__)
...
1000 continue                  ! Jumping point
  ...
  if (allocated(flx_nst_up)) deallocate(flx_nst_up,stat=rcd)
  if(rcd /= 0) stop 'deallocate() failed for flx_nst_up'

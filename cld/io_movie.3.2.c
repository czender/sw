 ...
 * $Date$
 * $Locker:  $
 * $RCSfile: io_movie.3.2.c,v $
 * $Source: /home/zender/cvs/cld/io_movie.3.2.c,v $
 * $Id$
 * $State: Exp $
 * */
 ...
 * Initial revision. Works with old ncarg3.01 in my environment and
 * (bonus) also compiles correctly on francke under 3.2 environment
 * when my old libraries are included first! About to try to change
 * to completely 3.2 calls.
 *
 *  */
...
main(int argc,char **argv)
{
  ...
  Gcolr_rep colr_rep_white;
  Gcolr_rep colr_rep_black;
  Gint mem_unit;
  Gint ws_id;
  Gint ws_type;
  ...
  colr_rep_white.rgb.red=(Gfloat)1.;
  colr_rep_white.rgb.green=(Gfloat)1.;
  colr_rep_white.rgb.blue=(Gfloat)1.;

  colr_rep_black.rgb.red=(Gfloat)0.;
  colr_rep_black.rgb.green=(Gfloat)0.;
  colr_rep_black.rgb.blue=(Gfloat)0.;
  ...
  while((opt = getopt(argc, argv,"c:D:d:Ee:Ii:Oo:Vv"))!= EOF){
    ...
    case 'v':
      ...
      (void)fprintf(stderr,"$RCSfile: io_movie.3.2.c,v $\n");
      (void)fprintf(stderr,"$Source: /home/zender/cvs/cld/io_movie.3.2.c,v $\n");
...
/*		    cloud_base_of_time[step*rad_step]*KILOMETER_PER_METER,*/
  ...
  gset_colr_rep((Gint)ws_id,(Gint)0,&colr_rep_white);  
/*  c_gscr(1,0,1.,1.,1.);*/
  /* this makes color 1 black */ 
  gset_colr_rep((Gint)ws_id,(Gint)1,&colr_rep_black);  
/*  c_gscr(1,1,0.,0.,0.);*/
  ...
  if(debug == 12){
	...
	ordinate[layer]=altitude[layer]*KILOMETERS_PER_METER; /* km */
      ...
      (void)sprintf(line_label,"Elapsed time :PGU1:D:RU:t = %.1f min.", 
		    time_snapshot[frame]*MINUTES_PER_SECOND); 
...
/*      c_gsasf(iasf);*/
      ...
      /* force solid fill */ 
      gset_fill_int_style((Gfill_int_style)1);
/*      c_gsfais(1);*/
     ...
     int *nra,*iai,*iag,*nai;
{
  ...
  point_list.num_points=(Gint)((*nra)-1);
  for(point=0;point<=point_list.num_points-1;point++){
    point_list.points[point].x=*(xra+point);
    point_list.points[point].y=*(yra+point);
  ...
  if(ifll != 0){
    ...
    if(ifll > 0 && ifll < NUM_IWC_CONTOURS){
      gset_fill_colr_ind((Gint)(ifll+1));      
/*      c_gsfaci(ifll+1);*/
...
/*      c_gfa(*nra-1,xra,yra);*/
    ...
    if( point_list.points != NULL ) free(point_list.points);
...
/* define a set of grey scale color triples for colors 2 through 22 */ 
  ...
  /* this makes color 0 white */ 
  colr_rep.rgb.red=(Gfloat)1.;
  colr_rep.rgb.green=(Gfloat)1.;
  colr_rep.rgb.blue=(Gfloat)1.;
/*  gset_colr_rep((Gint)ws_id,(Gint)0,&colr_rep);  */
/*  c_gscr(1,0,1.,1.,1.);*/

  /* this makes color 1 black */ 
  colr_rep.rgb.red=(Gfloat)0.;
  colr_rep.rgb.green=(Gfloat)0.;
  colr_rep.rgb.blue=(Gfloat)0.;
/*  gset_colr_rep((Gint)ws_id,(Gint)1,&colr_rep);  */
/*  c_gscr(1,1,0.,0.,0.);*/

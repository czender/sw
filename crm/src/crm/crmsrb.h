...
c     Values of variables in these common blocks are set in routines radcsw(), radclw()
c     which are called by radctl() which is called by crm().
...
c     Absorbed fluxes
      real flx_SW_abs_atm(plond) ! [W m-2] Shortwave flux absorbed in atmosphere
      real flx_NIR_abs_atm(plond) ! [W m-2] NIR flux absorbed in atmosphere
      real flx_vsb_abs_atm(plond) ! [W m-2] Visible flux absorbed in atmosphere
      real flx_bnd_abs_atm(plond,bnd_nbr_SW) ! [W m-2] Band flux absorbed in atmosphere
      real flx_SW_abs_sfc(plond) ! [W m-2] Shortwave flux absorbed by surface
      real flx_NIR_abs_sfc(plond) ! [W m-2] NIR flux absorbed by surface
      real flx_vsb_abs_sfc(plond) ! [W m-2] Visible flux absorbed by surface
      real flx_bnd_abs_sfc(plond,bnd_nbr_SW) ! [W m-2] Band flux absorbed by surface
      common / crmabs /         ! crmabs is initialized in radcsw()

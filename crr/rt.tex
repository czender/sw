...
pdfcreator={Charlie Zender},
pdfproducer={Charlie Zender},
pdfkeywords={Radiative Transfer} {ESS 138} {ESS 238}
} % !\hypersetup
...
\trmidx{planetary albedo} or \trmidx{spherical albedo}, $\rfl$.
...
A blackbody is an idealized physical body that absorbs all radiation incident upon it
at all wavelengths and from any direction.
Planck's Law states that blackbodies radiate at a rate described by the Planck function (\ref{eqn:plk_frq_dfn}).
Earth does not cool to space as a perfect blackbody
of a single temperature and emissivity.
...
\label{eqn:ngl}
\end{eqnarray}
The \trmdfn{field of view} of an electromagnetic sensor, e.g., a telescope, is
most naturally measured by the solid angle subtended by its aperture.
...
\subsubsection[Planck's Law]{Planck's Law}\label{sxn:plk_fnc}\label{sxn:plk_law}
A blackbody is an idealized physical body that absorbs all radiation incident upon it
at wavelengths and from any direction.
Planck's Law describes the spectral density of radiation emitted by a blackbody in
thermal equilibrium, when there is no net exchange of energy or mass between
the body and its environment.
The expression that governs the emitted radiance in these conditions is called
the Planck Function $\plkfrq$.
The Planck Function governs the intensity of emitted blackbody radiation as a function of temperature and wavelength:
...
that emits a radiance $\ntn$ is, by definition, the temperature
...
Both characteristics are measured in terms of temperature, yet
neither is the actual thermodynamic temperature of the object---unless the
object is a blackbody.
These characteristic temperatures are most useful when the emissivity of the
object is unknown.
Inverting (\ref{eqn:plk_dfn}) yields the four analytic forms of
$\tptbrt$: 
\begin{subequations}
...
to indicate that the brightness temperature definitions apply to any radiances, not 
necessarily blackbody radiances.
...
\clearpage
\noindent%
Helpful constants in Remote Sensing from the \href{https://www.physics.nist.gov/cuu/Constants}{2022 CODATA recommended} values:
\begin{enumerate*}
\item Speed of light: $\cstspdlgt = 2.99792458 \times 10^{8}$\,\mxs
\item Planck's constant: $\cstplk = 6.62607015 \times 10^{-34}$\,\js
\item Stefan-Boltzmann constant: $\cststfblt = 5.670374419  \times 10^{-8}$\,\wxmSkQ
\item Boltzmann's constant: $\cstblt = 1.380649  \times 10^{-23}$\,\jxK
\item Base of the natural logarithm: $\me = 2.718281828$
\item Pi: $\mpi = 3.141592654$
\end{enumerate*}
Helpful equations:
\begin{eqnarray}
...
to~$\xxx$:
% fxm: convert this to wavelength
\begin{eqnarray}
...
\rdncstone & = \frac{2 \mpi \cstblt^{4} \tpt^{4} }{\cstspdlgt^{2} \cstplk^{3}} =  \frac{15 \cststfblt \tpt^ {4}}{\mpi^{4}}, \mkern8mu
...
The range $10 < \xxxone \le 25$ is not well-documented for (\ref{eqn:tyl_mcl_sln_opt}), though $\nnn \le 2$ terms suffices for 10-digit precision of (\ref{eqn:tyl_mcl_sln_fll}).
...
\end{eqnarray}
% 20221020: Unable to reproduce Salby's result
% ncap2 -O -s 'x=1.0;pi=4.0*atan(1.0);e3=gsl_sf_expint_En(3,x);mu=-x/(ln(2.0*pi*e3));theta=acos(mu);theta_dgr=180*theta/pi;print(x);print(pi);print(e3);print(mu);print(theta);print(theta_dgr)' ~/foo.nc
% x = 1, pi = 3.14159265359, e3 = 0.109691967198, mu = 0.659225348946, theta = 0.851008227579, theta_dgr = 48.7591797712
% mu is much closer to 2/3 than to 3/5
...
in \trmidx{radiative equilibrium}.
...
\trmidx{direct (or solar-beam) component}, $\ntndwndrcoftaungl$, and a \trmidx{diffuse 
...
The time-dependent Schr\"{o}dinger equation predicts that the relation
...
Since $\frqnot \gg 1$, and the integrand peaks near the origin,

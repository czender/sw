...
\begin{eqnarray} 
\label{eqn:msv_plk_dfn}
\flxmsvfrq & = & \flxplkfrq \times \left( 2\int_{0}^{1} \msv(\frq,\plrmu) \plrmu \,\dfr\plrmu \right) \\
\msvhmsfrq & = & \flxmsvfrq/\flxplkfrq
\label{eqn:msv_hms_frq_dfn}
\end{eqnarray}
Hence it can be useful to tabulate the parenthetical factor on the RHS of  (\ref{eqn:msv_plk_dfn})
to convert blackbody hemispheric irradiance $\flxplkfrq$ into non-blackbody 
hemispheric irradiance $\flxmsvfrq$.
We name this factor the hemispheric mean spectral emissivity, $\msvhmsfrq$ (\ref{eqn:msv_hms_frq_dfn}).

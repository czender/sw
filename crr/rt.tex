...
The effective greybody emissivity $msvnot$ of an object can be defined by the ratio of its emitted (not scattered, not total) upwelling irradiance to blackbody irradiance at the same temperature so that
\begin{eqnarray}
\msvnot & \equiv & \flxupwmsv/\flxupwplk \nonumber \\
\flxupwmsv & = & \msvnot \cststfblt \tpt^{4}
\label{eqn:msv_not_dfn}
\end{eqnarray}
Kirchoff's Law for Radiation requires that the emissivity $\msvnot$ (and emitted power $\flxmsv$) for a greybody in thermal equilibrium must equal the absorptivity $\srpnot$ (and absorbed power $\flxsrp$), otherwise the body would drift from thermal equilibrium. 
Moreover, energy conservation requires that radiation not absorbed by an object must be reflected.
Thus the portion of the radiant downwelling flux $\flxdwn$ that reflected into upwelling flux $\flxupwrfl$ by a greybody surface is the co-absorptivity/emissivity times the incident flux. 
The total surface upwelling flux $\flxupw$ is the sum of the emitted and reflected fluxes upwelling fluxes:
\begin{eqnarray}
\msv(\frq) & \equiv & \srp(\frq) \nonumber \\
\msvnot & \equiv & \srpnot \nonumber \\
\flxsrp & = & \msvnot \flxdwn = \srpnot \flxdwn \nonumber \\
\flxupwrfl & = & (1- \msvnot) \flxdwn = (1 - \srpnot) \flxdwn \nonumber \\
\flxupw & \equiv & \flxupwmsv + \flxupwrfl \nonumber \\
\flxupw & = & \msvnot \cststfblt \tpt^{4} +  (1- \msvnot) \flxdwn

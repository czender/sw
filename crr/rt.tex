...
The Maclaurin series used to derive (\ref{eqn:tyl_mcl_sln_opt}) have a radius of convergence of $|\xxx| < 1$ due to singularities of $\ln(1+\xxx)$ and $\ln(1-\xxx)$ at $\xxx = \pm 1$.
...
\kkk^{-2}\sum_{\kkk=1}^{\kkk=\infty} \me^{-\kkk\xxxone}[3\xxxone^{2} + 6\kkk^{-1}(\xxxone + \kkk^{-1})] \}
\end{eqnarray}
The hemispherically integrated irradiance $\flxupwplkbndzro$ emitted
...
\flxupwplkbnd
& = & \flxupwplkbndinf - \flxupwplkbndpinf \nonumber \\
& = & \rdncstone
\int_{\xxxone}^{\xxxtwo} 
\frac{\xxx^{3}}{\me^{\xxx} - 1} \,\dfr\xxx \nonumber \\
& = & \rdncstone \{
...
\end{eqnarray}
For efficiency, a spectral radiative transfer model divided into $\nnn$~contiguous bands monotonically increasing interface frequencies/wavenumbers $[\xxxone, \xxxtwo, \cdots, \xxxiii, \cdots \xxxnnn, \xxxnnnp]$ would first evaluate $\flxupwplkbndinf$ (\ref{eqn:plk_ntg_fnl}) at all $\nnn+1$ interfaces.
...
This way each $\flxupwplkbndinf$ need be evaluated only once per temperature value.

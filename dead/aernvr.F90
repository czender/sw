...
! aer_nvr_cmn_ini() is called by dead()
...
module aernvr             ! [mdl] Aerosol environmental properties
  ...
  real(r8) doy              ! [day] Day of year [1.0..367.0)
...
contains
  ...
  subroutine aer_nvr_cmn_cmd_ln_dfl() ! [fnc] Initialize aer_nvr_cmn command-line fields
    ...
    doy=135.0_r8              ! [day] Day of year [1.0..367.0)
    do j=1,plat
       lat_dgr(j)=40.0_r8        ! [dgr]
       lat(j)=lat_dgr(j)      ! [dgr]
       lat_rdn(j)=real(pi,r8)*lat_dgr(j)/180.0_r8 ! [rdn]
    end do                    ! end loop over lat
    do i=1,plon
       lon(i)=15.0_r8*(real(i,r8)-1.0_r8)      ! [dgr]
    end do                    ! end loop over lon
    do k=1,plevp              ! Loop ends at plevp
       levp(k)=101325.0_r8-1000.0_r8*real(plevp-k,r8) ! [Pa]
    ...
    do k=1,plevp              ! Loop ends at plevp
       do i=1,plon
          hgt_ntf(i,k)=100.0_r8-10.0_r8*real(k-1,r8) ! [m] Interface height above surface
    ...
    do k=1,plev
       do i=1,plon
          ...
          tpt_mdp(i,k)=300.0_r8+3.0_r8*real(k-1,r8) ! [K] Temperature
  ...
  subroutine aer_nvr_cmn_ini( & 
    ...
    ! Routine's basic purpose is to initialize variables non-command-line variables
    ! These variables may (or may not) depend on command-line/external variables
    ! However, they are not command line variables themselves
    ! Routine must be called each time an independent variable is updated
    ! This may occur every timestep when fl_in is specified
    ...
    ! Parameters
    ! Input
    integer,intent(in)::nstep ! I [idx] Timestep index
    ! Local
    integer i                 ! [idx] Counting index for longitude
    integer k                 ! [idx] Counting index for level
    ...
    ! NB: It would be nice to make SW and LW fluxes externally-settable parameters
    do i=1,plon
    ...
    ! Set dust mixing ratio first timestep only. After this, prognose it.
    if (nstep == 0) then
       do m=1,dst_nbr
          do k=1,plev
             do i=1,plon
                ...
                q_cst(i,k,dst_idx_srt+m-1)=q_dst(i,k,m) ! [kg kg-1] Dust mixing ratio
             end do              ! end loop over lon
          end do                 ! end loop over lev
       end do                    ! end loop over cst
    endif ! endif first timestep
    ...
    ! Set water vapor mixing ratio portion of constituent array 
    ! This varies only when supplied by external forcing dataset
    ! CCM, MATCH differ store water vapor in opposite ends of constituent array
#ifdef CCM
    m=1
#else /* !CCM */
    m=pcnst
    ...
    do k=1,plev
       do i=1,plon
          ! CCM, MATCH both store water vapor as specific humidity [kg H2O vapor kg-1 moist air]

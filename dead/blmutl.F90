...
contains
  ...
  subroutine blm_ocn( & ! [sbr] Compute boundary layer properties over ocean
...
#ifdef FLUX_DIAGNOSTICS
    ! Variables used in diagnostics
    real(r8) flx_LW_upw_sfc(plond) ! [W m-2] Longwave upwelling flux at surface
...
#else /* !FLUX_DIAGNOSTICS */
    real(r8) CEWI_flt ! CEWI
    CEWI_flt=sum(dns_mdp)+sum(tpt_mdp) ! CEWI
#endif /* !FLUX_DIAGNOSTICS */
    ...
    ! Main Code
    xch_cff_mmn_sqrt=-1.0e36_r8 ! CEWI
    xch_cff_heat_sqrt=-1.0e36_r8 ! CEWI
    xch_cff_vpr_sqrt=-1.0e36_r8 ! CEWI
       ...
       if (flg_ocn(lon_idx)) then
          ...
          do itr_idx=1,itr_max
             ...
             if (.true.) then
                ...
                flx_LW_upw_sfc(lon_idx)=cst_Stefan_Boltzmann*(tpt_sfc(lon_idx)**4.0_r8) ! [W m-2] Longwave upwelling flux to atmosphere
                write (6,'(i3,1x,f9.3,1x,f8.3,1x,f7.4,1x,f7.3,1x,f7.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.6)') &
                     ...
                     flx_sns(lon_idx),flx_ltn(lon_idx),flx_LW_upw_sfc(lon_idx),eps_crr
             endif            ! endif dbg
#endif /* !FLUX_DIAGNOSTICS */

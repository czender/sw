...
contains
  ...
  subroutine rss_aer_get( & ! [sbr] Compute aerodynamic resistance
    ...
    real(r8),intent(in)::wnd_frc(plond)       ! [m s-1] Surface friction velocity from pbldif()
    real(r8),intent(in)::hgt_mdp(plond)       ! [m] Midlayer height above surface
    real(r8),intent(in)::hgt_zpd(plond)       ! [m] Zero plane displacement
    ! Output
    real(r8),intent(out)::rss_aer(plond)       ! [s m-1] Aerodynamic resistance
    ! Local
    ...
    do lon_idx=1,plon
       if (rgh_mmn(lon_idx) <= 0.0.or.hgt_mdp(lon_idx) <= hgt_zpd(lon_idx)) then
          write(6,'(a,3(a,es8.1),a)') 'rss_aer_get: ','rgh_mmn = ',rgh_mmn(lon_idx), &
               ', hgt_mdp = ',hgt_mdp(lon_idx),' m, hgt_zpd = ',hgt_zpd(lon_idx),' m s-1'
       endif                  ! endif err
    ...
    do lon_idx=1,plon
       ...
       if (mno_lng(lon_idx) < 0.0_r8) then
  ...
  subroutine rss_lmn_get( & ! [sbr] Compute resistance of quasi-laminar layer 
    ...
    real(r8),parameter::shm_nbr_xpn_lnd=-2.0/3.0 ! [frc] Exponent for aerosol-diffusion dependence on Schmidt number over land
    real(r8),parameter::shm_nbr_xpn_ocn=-0.5 ! [frc] Exponent for aerosol-diffusion dependence on Schmidt number over ocean
    ! Input
    real(r8),intent(in)::dns_aer(dst_nbr)     ! I [kg m-3] Particle density
    real(r8),intent(in)::dmt_aer(dst_nbr)     ! I [m] Particle diameter
    real(r8),intent(in)::stk_crc(dst_nbr)     ! I [frc] Correction to Stokes settling velocity
    ...
    real(r8),intent(in)::wnd_frc(plond)       ! I [m s-1] Friction velocity
    ! Output
    real(r8),intent(out)::rss_lmn(plond,dst_nbr) ! O [s m-1] Quasi-laminar layer resistance
    real(r8),intent(out)::stk_nbr(plond,dst_nbr) ! O [frc] Stokes number
    real(r8),intent(out)::shm_nbr(plond,dst_nbr) ! O [frc] Schmidt number
    ! Local
    real(r8) pi               ! [frc] 3
    ...
    real(r8) tmp_do_not_underflow ! [frc] Factor in rss_lmn computation
    ...
    do i=1,plon
       vsc_dyn_atm(i)=1.72e-5_r8*((tpt_mdp(i)/273.0_r8)**1.5_r8)*393.0_r8/(tpt_mdp(i)+120.0_r8) ! [kg m-1 s-1] RoY94  p. 102
       mfp_atm(i)=2.0_r8*vsc_dyn_atm(i)/(prs_mdp(i)*sqrt(8.0_r8*mmw_dry_air/(pi*gas_cst_unv*tpt_mdp(i)))) ! [m] Mean free path of air SeP97 p. 455
       vsc_knm_atm(i)=vsc_dyn_atm(i)/dns_mdp(i) ! [m2 s-1] Kinematic viscosity of air
    end do                    ! end loop over lon
    
    do m=1,dst_nbr
       do i=1,plon
          slp_crc(i,m)=1.0_r8+2.0_r8*mfp_atm(i)*(1.257_r8+0.4_r8*exp(-1.1_r8*dmt_aer(m)/(2.0_r8*mfp_atm(i))))/dmt_aer(m) ! [frc] Slip correction factor SeP97 p. 464
          vlc_grv(i,m)=(1.0_r8/18.0_r8)*dmt_aer(m)*dmt_aer(m)*dns_aer(m)*grv_sfc*slp_crc(i,m)/vsc_dyn_atm(i) ! [m s-1] Stokes' settling velocity SeP97 p. 466
    ...
    do m=1,dst_nbr
       do i=1,plon
          ...
          dff_aer(i,m)=cst_Boltzmann*tpt_mdp(i)*slp_crc(i,m)/(3.0_r8*pi*vsc_dyn_atm(i)*dmt_aer(m)) ! [m2 s-1] Brownian diffusivity of particle SeP97 p. 474
          ...
          ! 20070219: Fix single precision underflow
          if (stk_nbr(i,m) > 0.10) then
             tmp_do_not_underflow=10.0_r8**(-3.0_r8/stk_nbr(i,m))
          else 
             tmp_do_not_underflow=1.0e-30_r8 
          endif ! end underflow circumventor
          tmp=tmp_do_not_underflow+shm_nbr(i,m)**shm_nbr_xpn
          rss_lmn(i,m)=1.0_r8/(tmp*wnd_frc(i)) ! [s m-1] SeP97 p. 972, 965
       end do                 ! end loop over lon
  ...
  subroutine stk_crc_get( & ! [sbr] Determine bias in Stokes settling velocity
    ...
    do sz_idx=1,sz_nbr
       ...
       do while(eps_crr > eps_max)
          ...
          if (itr_idx > 20) then
             write (6,'(a)') 'dead: Terminal velocity not converging in stk_crc_get(), breaking loop...'
...
100    continue               ! Label to jump to when iteratio does not converge
  ...
  subroutine vlc_grv_get( & ! [sbr] Compute terminal fall speed
    ...
    real(r8),intent(in)::dmt_prt(dst_nbr)     ! I [m] Particle diameter
    real(r8),intent(in)::prs_mdp(plond,plev)  ! I [Pa] Pressure 
    real(r8),intent(in)::tpt_mdp(plond,plev)  ! I [K] Temperature
    real(r8),intent(in)::stk_crc(dst_nbr)     ! I [frc] Correction to Stokes settling velocity
    ! Output
    real(r8),intent(out)::vlc_grv(plond,plev,dst_nbr) ! O [m s-1] Settling velocity
    ! Local
    real(r8) pi       ! [frc] 3
    integer i                 ! [idx] Counting index
    ...
    do k=1,plev
       do i=1,plon
          vsc_dyn_atm(i,k)=1.72e-5_r8*((tpt_mdp(i,k)/273.0_r8)**1.5_r8)*393.0_r8/(tpt_mdp(i,k)+120.0_r8) ! [kg m-1 s-1] RoY94  p. 102
          mfp_atm(i,k)=2.0_r8*vsc_dyn_atm(i,k)/(prs_mdp(i,k)*sqrt(8.0_r8*mmw_dry_air/(pi*gas_cst_unv*tpt_mdp(i,k)))) ! [m] Mean free path of air SeP97 p. 455
       end do                 ! end loop over lon
    ...
    do m=1,dst_nbr
       do k=1,plev
          do i=1,plon
             slp_crc=1.0_r8+2.0_r8*mfp_atm(i,k)*(1.257_r8+0.4_r8*exp(-1.1_r8*dmt_prt(m)/(2.0_r8*mfp_atm(i,k))))/dmt_prt(m) ! [frc] Slip correction factor SeP97 p. 464
             vlc_grv(i,k,m)=(1.0_r8/18.0_r8)*dmt_prt(m)*dmt_prt(m)*dns_prt(m)*grv_sfc*slp_crc/vsc_dyn_atm(i,k) ! [m s-1] Stokes settling velocity SeP97 p. 466

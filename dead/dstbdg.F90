...
contains
  ...
  subroutine bdg_cmn_ini(grv_sfc,lat_wgt_tmp,tm_dlt_tmp)
    ...
    implicit none
    ! Parameters
    character(len=*),parameter::sbr_nm='bdg_cmn_ini' ! [sng] Subroutine name
    ! Input
    ...
    integer::fl_out_fmt=nf90_format_classic ! [enm] Output file format
    ...
    grv_sfc_plon_rcp=1.0_r8/(real(plon)*grv_sfc) ! [s2 m-1] 1.0/(grv_sfc*plon)
    ...
    ! Define dimensions
    rcd=rcd+nf90_def_dim(nc_id,'time',nf90_unlimited,time_dmn_id)
    ! Define variables
    nf90_r8=nf90_xtype_r8_get() ! [enm] External netCDF type for r8 kind
    rcd=rcd+nf90_def_var(nc_id,'nstep',nf90_int,time_dmn_id,var_id(1))
    rcd=rcd+nf90_def_var(nc_id,'date',nf90_int,time_dmn_id,var_id(2))
    rcd=rcd+nf90_def_var(nc_id,'datesec',nf90_int,time_dmn_id,var_id(3))
    do var_idx=1,var_nbr
       rcd=rcd+nf90_def_var(nc_id,var_lst(var_idx),nf90_r8,time_dmn_id,var_id(3+var_idx))
    end do                    ! end loop over var
    ! Derived budget fields defined in bdg_update()
    rcd=rcd+nf90_def_var(nc_id,'dst_dry_dlt',nf90_r8,time_dmn_id,dst_dry_dlt_id)
    rcd=rcd+nf90_def_var(nc_id,'dst_mbl_dlt',nf90_r8,time_dmn_id,dst_mbl_dlt_id)
    rcd=rcd+nf90_def_var(nc_id,'dst_pcp_dlt',nf90_r8,time_dmn_id,dst_pcp_dlt_id)
    rcd=rcd+nf90_def_var(nc_id,'dst_sf_dps',nf90_r8,time_dmn_id,dst_sf_dps_id)
    rcd=rcd+nf90_def_var(nc_id,'dst_sf_net',nf90_r8,time_dmn_id,dst_sf_net_id)
    rcd=rcd+nf90_def_var(nc_id,'flx_mss_dps',nf90_r8,time_dmn_id,flx_mss_dps_id)
    rcd=rcd+nf90_def_var(nc_id,'flx_mss_dry',nf90_r8,time_dmn_id,flx_mss_dry_id)
    rcd=rcd+nf90_def_var(nc_id,'flx_mss_grv',nf90_r8,time_dmn_id,flx_mss_grv_id)
    rcd=rcd+nf90_def_var(nc_id,'flx_mss_mbl',nf90_r8,time_dmn_id,flx_mss_mbl_id)
    rcd=rcd+nf90_def_var(nc_id,'flx_mss_pcp',nf90_r8,time_dmn_id,flx_mss_pcp_id)
    rcd=rcd+nf90_def_var(nc_id,'flx_mss_trb',nf90_r8,time_dmn_id,flx_mss_trb_id)
    rcd=rcd+nf90_def_var(nc_id,'mpc_dst_dgn',nf90_r8,time_dmn_id,mpc_dst_dgn_id)
    rcd=rcd+nf90_def_var(nc_id,'mpc_dst_dlt',nf90_r8,time_dmn_id,mpc_dst_dlt_id)
    rcd=rcd+nf90_def_var(nc_id,'mpc_dst_mdl_avg',nf90_r8,time_dmn_id,mpc_dst_mdl_avg_id)
    rcd=rcd+nf90_def_var(nc_id,'mpc_err',nf90_r8,time_dmn_id,mpc_err_id)
    rcd=rcd+nf90_def_var(nc_id,'mpc_err_frc',nf90_r8,time_dmn_id,mpc_err_frc_id)
    rcd=rcd+nf90_def_var(nc_id,'mpc_err_tnd_frc',nf90_r8,time_dmn_id,mpc_err_tnd_frc_id)
    rcd=rcd+nf90_def_var(nc_id,'tau_dps',nf90_r8,time_dmn_id,tau_dps_id)
    rcd=rcd+nf90_def_var(nc_id,'tau_dry',nf90_r8,time_dmn_id,tau_dry_id)
    rcd=rcd+nf90_def_var(nc_id,'tau_grv',nf90_r8,time_dmn_id,tau_grv_id)
    rcd=rcd+nf90_def_var(nc_id,'tau_mbl',nf90_r8,time_dmn_id,tau_mbl_id)
    rcd=rcd+nf90_def_var(nc_id,'tau_pcp',nf90_r8,time_dmn_id,tau_pcp_id)
    rcd=rcd+nf90_def_var(nc_id,'tau_trb',nf90_r8,time_dmn_id,tau_trb_id)
    rcd=rcd+nf90_def_var(nc_id,'tm_ttl',nf90_r8,time_dmn_id,tm_ttl_id)
  ...
  subroutine bdg_gam_dry(var_nm,lat_idx,prs_dlt,q_H2O_vpr_mmrm,mmrd)
    ! Purpose: Calculate latitude contribution to global average mass of tracer field
    ! Input units of tracer are dry mass mixing ratio (kg tracer / kg dry air)
    ! Input units of specific humidity moist mass mixing ratio (kg H2O vapor/ kg moist air)
    ! Output masses are in [kg m-2]
    ! bdg_gam_dry() is called by host model once for each latitude slice for each 3D mmrd diagnostic field each timestep
    use shr_kind_mod,only:r8=>shr_kind_r8 ! [mdl] Precision r8, i8, ...
    ...
    mss_lat(lat_idx,var_idx)=0.0_r8 ! [kg m-2] Mass path of tracer (modulo pre-factors)
    do lev_idx=1,plev
       do lon_idx=1,plon
          mss_lat(lat_idx,var_idx)=mss_lat(lat_idx,var_idx)+ &
               mmrd(lon_idx,lev_idx)*(1.0_r8-q_H2O_vpr_mmrm(lon_idx,lev_idx))*prs_dlt(lon_idx,lev_idx) ! [kg m-2] Mass path of tracer (modulo pre-factors)
  ...
  subroutine bdg_gam_wet(var_nm,lat_idx,prs_dlt,mmrm)
    ! Purpose: Calculate latitude contribution to global average mass of tracer field
    ! Input units of tracer are moist mass mixing ratio (kg tracer / kg moist air)
    ! Output masses are in [kg m-2]
    ! bdg_gam_wet() is called by host model once for each latitude slice for each 3D mmrm diagnostic field each timestep
    use shr_kind_mod,only:r8=>shr_kind_r8 ! [mdl] Precision r8, i8, ...
    use pmgrid ! [mdl] Spatial resolution parameters
    implicit none
    ! Input
  ...
  subroutine bdg_gam_wet_2D(var_nm,lat_idx,prs_dlt,mmrm)
    ! Purpose: Calculate latitude contribution to global average mass of a surface tracer field
    ! Input concentration units are moist mixing ratio (kg tracer / kg moist air)
    ! Output masses are in [kg m-2]
    ! bdg_gam_wet_2D() is called by host model once for each latitude slice for each 2D mmrm diagnostic field each timestep
    use shr_kind_mod,only:r8=>shr_kind_r8 ! [mdl] Precision r8, i8, ...
    use pmgrid ! [mdl] Spatial resolution parameters
    implicit none
    ! Input
  ...
  subroutine bdg_aal(var_nm,lat_idx,var_dat)
    ! Purpose: Store in memory the latitude contribution to area average of field
    ! bdg_aal() takes zonal average of latitude slice of a 2D field and stores it
    ! bdg_gam() is called by host model once for each latititude slice for each 2D diagnostic field each timestep
    use shr_kind_mod,only:r8=>shr_kind_r8 ! [mdl] Precision r8, i8, ...
    use pmgrid ! [mdl] Spatial resolution parameters
    implicit none
    ! Input
  ...
  real(r8) function bdg_aa(var_nm)
    ...
    bdg_aa=bdg_aa*0.5_r8/real(plon)    ! [kg m-2]
    return
  ...
  subroutine bdg_update( &
    ...
    wgt_pst=(real(time_idx)-1.0_r8)/real(time_idx) ! [frc] Weight of past in running average (fragile)
    wgt_crr=1.0_r8/real(time_idx)      ! [frc] Weight of current timestep in running average (fragile)
  ...
  subroutine qneg3_dgn( &
    ...
    ! Purpose: Diagnostic plugin replacement for qneg3() routine
    ! Computes and archives tendencies due to qneg3()
    ! Dependencies: Requires qneg3()
    use pmgrid ! [mdl] Spatial resolution parameters
    implicit none
    ! Input
    ...
    ! Main code
    ! Same array holds initial value q_mmr and qneg3() tendency to save memory
    qneg3_tnd=q_mmr           ! [kg kg-1] Constituent array before qneg3()
    call qneg3(sbr_nm,lat_idx,q_mmr)
    ! Compute tendency due to qneg3()
    ! By definition this should be a postive tendency because mass after qneg3()
    ! is always greater than mass before
    qneg3_tnd=(q_mmr-qneg3_tnd)/tm_adj ! [kg kg-1 s-1] Tendency due to qneg3() limiter
    ! Archive tendency to global mass diagnostics
#ifdef DST_MSS_BDG

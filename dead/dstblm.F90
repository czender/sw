...
contains
  ...
  subroutine hyd_prp_get( &
    ...
    real(r8),intent(in)::mss_frc_snd(plond)   ! I [frc] Mass fraction sand
    ! Output
    real(r8),intent(out)::vwc_dry(plond)       ! O [m3 m-3] Dry volumetric water content (no E-T)
    real(r8),intent(out)::vwc_opt(plond)       ! O [m3 m-3] E-T optimal volumetric water content 
    real(r8),intent(out)::vwc_sat(plond)       ! O [m3 m-3] Saturated volumetric water content (sand-dependent)
    ! Local
    ...
    do lon_idx=1,plond        ! NB: plond
       vwc_dry(lon_idx)=0.0_r8   ! [m3 m-3] Dry volumetric water content (no E-T)
       vwc_opt(lon_idx)=0.0_r8   ! [m3 m-3] E-T optimal volumetric water content 
       vwc_sat(lon_idx)=0.0_r8   ! [m3 m-3] Saturated volumetric water content (sand-dependent)
    ...
    do lon_idx=1,plon
       if (flg_mbl(lon_idx)) then
          smp_xpn_b(lon_idx)= & ! [frc] Exponent "b" for smp (clay-dependent)
               2.91_r8+0.159_r8*mss_frc_cly(lon_idx)*100.0_r8
          ...
          smp_sat(lon_idx)=   & ! [mm H2O] Saturated soil matric potential (sand-dependent)
               10.0_r8*(10.0_r8**(1.88_r8-0.0131_r8*mss_frc_snd(lon_idx)*100.0_r8))
          vwc_sat(lon_idx)=   & ! [m3 m-3] Saturated volumetric water content (sand-dependent)
               0.489_r8-0.00126_r8*mss_frc_snd(lon_idx)*100.0_r8
          vwc_dry(lon_idx)=   & ! [m3 m-3] Dry volumetric water content (no E-T)
               vwc_sat(lon_idx)* &
               (316230.0_r8/smp_sat(lon_idx))**(-1.0_r8/smp_xpn_b(lon_idx))
          vwc_opt(lon_idx)=   & ! [m3 m-3] E-T optimal volumetric water content
               vwc_sat(lon_idx)* &
               (158490.0_r8/smp_sat(lon_idx))**(-1.0_r8/smp_xpn_b(lon_idx))
       endif                  ! endif flg_mbl
  ...
  subroutine cnd_trm_soi_get( &
    ...
    real(r8),intent(in)::mss_frc_cly(plond)   ! I [frc] Mass fraction clay 
    real(r8),intent(in)::mss_frc_snd(plond)   ! I [frc] Mass fraction sand
    ! real(r8),intent(in)::snw_hgt(plond)       ! I [m] Geometric bulk thickness of snow
    real(r8),intent(in)::tpt_soi(plond)       ! I [K] Soil temperature
    real(r8),intent(in)::vwc_dry(plond)       ! I [m3 m-3] Dry volumetric water content (no E-T)
    real(r8),intent(in)::vwc_opt(plond)       ! I [m3 m-3] E-T optimal volumetric water content
    real(r8),intent(in)::vwc_sat(plond)       ! I [m3 m-3] Saturated volumetric water content
    real(r8),intent(in)::vwc_sfc(plond)       ! I [m3 m-3] Volumetric water content
    ! Output
    real(r8),intent(out)::cnd_trm_soi(plond)   ! O [W m-1 K-1] Soil thermal conductivity
    real(r8),intent(out)::lvl_dlt(plond)       ! O [m] Soil layer thickness
    ! Local
    integer lon_idx           ! [idx] Counting index
    ...
    ! Main Code
    ! Initialize arrays
    CEWI_flt=sum(vwc_dry)+sum(vwc_opt) ! CEWI
    ...
    do lon_idx=1,plon
       if (flg_mbl(lon_idx)) then
          ...
          cnd_trm_soi_sld(lon_idx)= & ! [W m-1 K-1] Thermal conductivity of soil solids CCM:lsm/lsmtci() Bon96 p. 77
               (8.80_r8*mss_frc_snd(lon_idx)+2.92_r8*mss_frc_cly(lon_idx)) &
               /(mss_frc_snd(lon_idx)+mss_frc_cly(lon_idx))
          cnd_trm_soi_dry(lon_idx)=0.15_r8 ! [W m-1 K-1] Thermal conductivity of dry soil CCM:lsm/lsmtci() Bon96 p. 77
          cnd_trm_soi_wrm(lon_idx)= & ! [W m-1 K-1] Soil thermal conductivity, unfrozen
               +cnd_trm_soi_dry(lon_idx) &
               +(cnd_trm_soi_sld(lon_idx)**(1.0_r8-vwc_sat(lon_idx)) &
  ...
  subroutine trn_fsh_vpr_soi_atm_get( &
    ...
    real(r8),intent(in)::vwc_sfc(plond)       ! I [m3 m-3] Volumetric water content
    ! Output
    real(r8),intent(out)::trn_fsh_vpr_soi_atm(plond) ! O [frc] Transfer efficiency of vapor from soil to atmosphere
    ! Local
    integer lon_idx           ! [idx] Counting index
       ...
       if (flg_mbl(lon_idx)) then
          if (tpt_soi(lon_idx) > tpt_soi_frz) then
             trn_fsh_vpr_soi_atm(lon_idx)= & ! [frc] Transfer efficiency of vapor from soil to atmosphere
                  min(max(vwc_sfc(lon_idx)-vwc_dry(lon_idx),0.0_r8)/(vwc_opt(lon_idx)-vwc_dry(lon_idx)),1.0_r8) ! CCM:lsm/surphys Bon96 p. 59
          else
  ...
  subroutine blm_mbl( &
    ...
    ! Input
    logical,intent(in)::flg_mbl(plond)    ! I [flg] Mobilization candidate flag
    real(r8),intent(in)::cnd_trm_soi(plond)   ! I [W m-1 K-1] Soil thermal conductivity
    real(r8),intent(in)::dns_mdp(plond)       ! I [kg m-3] Midlayer density
    real(r8),intent(in)::flx_LW_dwn_sfc(plond) ! I [W m-2] Longwave downwelling flux at surface
    real(r8),intent(in)::flx_SW_abs_sfc(plond) ! I [W m-2] Solar flux absorbed by ground
    real(r8),intent(in)::hgt_mdp(plond)       ! I [m] Midlayer height above surface
    real(r8),intent(in)::hgt_zpd              ! I [m] Zero plane displacement
    real(r8),intent(in)::lvl_dlt(plond)       ! I [m] Soil layer thickness
    real(r8),intent(in)::prs_mdp(plond)       ! I [Pa] Pressure
    real(r8),intent(in)::q_H2O_vpr(plond)     ! I [kg kg-1] Specific humidity
    real(r8),intent(in)::rgh_mmn              ! I [m] Roughness length momentum
    real(r8),intent(in)::trn_fsh_vpr_soi_atm(plond) ! I [frc] Transfer efficiency of vapor from soil to atmosphere
    real(r8),intent(in)::tpt_mdp(plond)       ! I [K] Midlayer temperature
    real(r8),intent(in)::tpt_ptn_mdp(plond)   ! I [K] Midlayer local potential temperature (relative to surface pressure, not to 1000 mb)
    real(r8),intent(in)::tpt_soi(plond)       ! I [K] Soil temperature
    !real(r8),intent(in)::wnd_mrd_mdp(plond)   ! I [m s-1] Surface layer meridional wind speed
    !real(r8),intent(in)::wnd_znl_mdp(plond)   ! I [m s-1] Surface layer zonal wind speed
    real(r8),intent(in):: wnd_mdp(plond)       !I [m s-1] Surface layer wind speed 
    ! Input/Output
    real(r8),intent(inout)::tpt_gnd(plond)       ! I/O [K] Ground temperature
    ! Output
    real(r8),intent(out)::mno_lng(plond)       ! O [m] Monin-Obukhov length
    real(r8),intent(out)::wnd_frc(plond)       ! O [m s-1] Surface friction velocity
    ! Local
    ...
    do lon_idx=1,plon
       if (flg_mbl(lon_idx)) then
          ...
          mno_stb_prm(lon_idx)=min(-(hgt_mdp(lon_idx)-hgt_zpd)/15.0_r8,1.0_r8) ! [frc]
          rgh_heat(lon_idx)=rgh_mmn ! [m] Roughness length heat
          tpt_vrt(lon_idx)=tpt_mdp(lon_idx)*(1.0_r8+eps_H2O_rcp_m1*q_H2O_vpr(lon_idx)) ! [K] Virtual temperature
       endif                  ! endif flg_mbl
    end do                     ! end loop over lon
    ...
    do lon_idx=1,plon
       if (flg_mbl(lon_idx)) then
          ...
          tpt_gnd_dlt(lon_idx)=0.0_r8 ! [K] Change in ground temperature
    ...
    do lon_idx=1,plon
       if (flg_mbl(lon_idx)) then
          ...
          mno_stb_prm_old(lon_idx)=0.0_r8 ! [frc] Monin Obukhov stability parameter old
          ! Stability-independent components of aerodynamic resistance calculations
          rss_aer_fct(lon_idx)=1.0_r8/(cst_von_krm*cst_von_krm*wnd_mdp_bnd(lon_idx)) ! [s m-1]
    ...
    do lon_idx=1,plon
       if (flg_mbl(lon_idx)) then
          ...
          flx_sns_gnd_cff_b(lon_idx)=2.0_r8*cnd_trm_soi(lon_idx)/lvl_dlt(lon_idx) ! [W m-2 K-1] b in Fgnd = a + b*Tg Bon96 p. 64
    ...
    do itr_idx=1,itr_max
       ...
       do lon_idx=1,plon
          if (flg_mbl(lon_idx)) then
             ...
             if (mno_stb_prm(lon_idx) < 0.0_r8) then
                sml_fnc_mmn_uns_rcp=(1.0_r8-16.0_r8*mno_stb_prm(lon_idx))**0.25_r8
                ...
                mno_stb_crc_mmn_crr=-5.0_r8*mno_stb_prm(lon_idx) ! [frc]
             ...
             rss_aer_mmn(lon_idx)=max(rss_aer_fct(lon_idx)*mno_stb_crc_tmp4*mno_stb_crc_tmp4,1.0_r8) ! [s m-1]
             rss_aer_heat(lon_idx)=max(rss_aer_fct(lon_idx)*mno_stb_crc_tmp4*mno_stb_crc_tmp5,1.0_r8) ! [s m-1]
             ...
             cff_xch_mmn(lon_idx)=1.0_r8/(rss_aer_mmn(lon_idx)*wnd_mdp_bnd(lon_idx)) ! [frc]
             ! Friction velocity
          ...
          do lon_idx=1,plon
             if (flg_mbl(lon_idx)) then
                rss_sfc_vpr(lon_idx)= & ! [s m-1] Surface resistance to vapor transfer Bon96 p. 56, Fgr. 17 p. 57
                     rss_aer_vpr(lon_idx)*(1.0_r8-trn_fsh_vpr_soi_atm(lon_idx))/ &
       ...
       do lon_idx=1,plon
          if (flg_mbl(lon_idx)) then
             ...
             cnd_heat_sfc_mdp=1.0_r8/rss_aer_heat(lon_idx) ! [m s-1] Sensible heat conductance surface air to midlayer air Bon96 p. 60, Fig. 16 p. 57
       ...
       do lon_idx=1,plon
          if (flg_mbl(lon_idx)) then
             ...
             cnd_vpr_sfc_mdp=1.0_r8/rss_aer_vpr(lon_idx) ! [m s-1] Water vapor conductance surface air to midlayer air Bon96 p. 60, Fgr. 17 p. 57
             cnd_vpr_gnd_sfc=1.0_r8/rss_sfc_vpr(lon_idx) ! [m s-1] Water vapor conductance ground to surface air Bon96 p. 60, Fgr. 17 p. 57
       ...
       do lon_idx=1,plon
          if (flg_mbl(lon_idx)) then
             ...
             flx_LW_net(lon_idx)=flx_LW_net_cff_a(lon_idx)+flx_LW_net_cff_b(lon_idx)*(tpt_gnd(lon_idx)**4.0_r8) ! [W m-2] Net longwave flux to atmosphere
             ...
             nrg_bdg_dlt=     & ! [W m-2 K-1] Temperature derivative of surface energy budget
                  -4.0_r8*flx_LW_net_cff_b(lon_idx)*(tpt_gnd(lon_idx)**3.0_r8) &
       ...
       do lon_idx=1,plon
          if (flg_mbl(lon_idx)) then
             ...
             mno_lng(lon_idx)=-1.0_r8*(wnd_frc(lon_idx)**3.0_r8)/mno_dnm ! [m] Monin-Obukhov length Bon96 p. 49
             ! Stability functions only valid for stability parameter zeta <= 1.0 Bon96 p. 52, Bru82
             mno_stb_prm(lon_idx)=min((hgt_mdp(lon_idx)-hgt_zpd)/mno_lng(lon_idx),1.0_r8) ! [frc] Monin Obukhov stability parameter
             
             ! Accumulate number of times stability parameter changes sign 
             if (mno_stb_prm_old(lon_idx)*mno_stb_prm(lon_idx) < 0.0_r8) sgn_chg_ctr(lon_idx)=sgn_chg_ctr(lon_idx)+1
             ! Zero stability parameter if it has changed sign too many times
             if (sgn_chg_ctr(lon_idx) >= sgn_chg_ctr_max) then
                mno_stb_prm(lon_idx)=0.0_r8 ! [frc]
                ! Zero stability corrections for consistency with stability parameter
                mno_stb_crc_mmn(lon_idx)=0.0_r8 ! [frc]
                mno_stb_crc_heat(lon_idx)=0.0_r8 ! [frc]
             endif            ! endif

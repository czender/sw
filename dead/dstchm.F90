...
contains
  ...
  subroutine dst_chm_cmn_ini()
    ...
    ! Based on Jack Lunsford Texas A&M measurements of OH uptake on dry Fe2O3
    upt_cff_OH_dst=1.0e-5_r8     ! [frc] Uptake coefficient for OH to dust
  ...
  subroutine dst_chm_rxr(lchnk,ncol,obuf, &
    ...
    ! Main Code
    CEWI_flt=real(lat_idx)+real(lchnk)+real(ncol)+sum(dns_mdp)+sum(prs_mdp)+sum(tpt_vrt)+obuf(1) ! CEWI
  ...
  subroutine dst_chm_slv(lchnk,ncol,obuf, &
    ...
    ! Main Code
    CEWI_flt=real(lchnk)+real(ncol)+obuf(1) ! CEWI
    ...
    do k=1,plev
       do i=1,plon
          if (vmr_HNO3_gas(i,k) > vmr_HNO3_gas_mxm) write(6,'(a,i2,a,i3,a,i2,a,es8.1,a)')  &
               'dst_chm_slv: lat = ',lat_idx,' vmr_HNO3_gas(',i,',',k,') = ',vmr_HNO3_gas(i,k),' mlc mlc-1'
       end do                 ! end loop over lon
    end do                    ! end loop over lev
#endif /* not DST_DBG */
  ...
  subroutine chm2nc(             &
    ...
    if (nstep == 1) then
       ...
       rcd=rcd+nf90_def_var(nc_id,'vmr_HNO3_gas',nf90_float,dmn_lon_lev_time,vmr_HNO3_gas_id)
       rcd=rcd+nf90_def_var(nc_id,'vmr_HO2_gas',nf90_float,dmn_lon_lev_time,vmr_HO2_gas_id)
       rcd=rcd+nf90_def_var(nc_id,'vmr_N2O5_gas',nf90_float,dmn_lon_lev_time,vmr_N2O5_gas_id)
       rcd=rcd+nf90_def_var(nc_id,'vmr_O3_gas',nf90_float,dmn_lon_lev_time,vmr_O3_gas_id)
       rcd=rcd+nf90_def_var(nc_id,'vmr_SO2_gas',nf90_float,dmn_lon_lev_time,vmr_SO2_gas_id)
       rcd=rcd+nf90_def_var(nc_id,'vmr_NO3_aer',nf90_float,dmn_lon_lev_time,vmr_NO3_aer_id)
       rcd=rcd+nf90_def_var(nc_id,'vmr_SO4_aer',nf90_float,dmn_lon_lev_time,vmr_SO4_aer_id)
       rcd=rcd+nf90_def_var(nc_id,'rxr_HNO3_gas_dst_vmr',nf90_float,dmn_lon_lev_time,rxr_HNO3_gas_dst_vmr_id)
       rcd=rcd+nf90_def_var(nc_id,'rxrc_HNO3_dst',nf90_float,dmn_lon_lev_time,rxrc_HNO3_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'rxrc_HO2_dst',nf90_float,dmn_lon_lev_time,rxrc_HO2_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'rxrc_N2O5_dst',nf90_float,dmn_lon_lev_time,rxrc_N2O5_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'rxrc_O3_dst',nf90_float,dmn_lon_lev_time,rxrc_O3_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'rxrc_SO2_dst',nf90_float,dmn_lon_lev_time,rxrc_SO2_dst_id)

...
contains
  ...
  subroutine dst_dps_wet(lchnk,ncol,obuf, & ! [sbr] Wet deposition driver
    ...
    ! Main Code
    CEWI_flt=sum(prs_mdp)+sum(q_H2O_cnd_cnv)+obuf(1) ! CEWI
    ...
    do k=1,plev
       do i=1,plon
          ...
          ! if (q_H2O_pcp2vpr_tnd(i,k) < 0.0_r8) write(6,100) 'dead: dst_dps_wet: lat = ',lat_idx,' q_H2O_pcp2vpr_tnd(',i,',',k,') = ',q_H2O_pcp2vpr_tnd(i,k)
          ! if (q_H2O_cnd2pcp_tnd(i,k) < 0.0_r8) write(6,100) 'dead: dst_dps_wet: lat = ',lat_idx,' q_H2O_cnd2pcp_tnd(',i,',',k,') = ',q_H2O_cnd2pcp_tnd(i,k)
          ...
          ! write(6,'(a)') 'dead: ERROR dst_dps_wet() reports cloud fraction error:'
       ...
       do k=1,plev
          do i=1,plon
             if (q_dst(i,k,m) < 0.0_r8 .or. q_dst(i,k,m) > q_dst_mxm) then
                write(6,'(a)') 'dead: ERROR dst_dps_wet() reports mixing ratio too high or too low:'
  ...
  subroutine cld_dgn( & ! [sbr] Cloud diagnostics
    ...
    ! Main Code
    CEWI_flt=sum(q_h2O_cnd_tnd) ! CEWI
  ...
  subroutine dst_dps_wet_old(lchnk,ncol,obuf, &
    ...
    ! Main Code
    CEWI_flt=sum(cld_frc)+sum(prs_mdp)+sum(tpt_mdp)+obuf(1) ! CEWI
    ! Timesplit if desired
    tm_dlt=tm_adj             ! [s] (default CCM: 2*dt, MATCH: dt)
          ...
          do i=1,plon
             ...
             if (tm_dlt*q_dst_tnd_pcp(i,k,m) < q_dst(i,k,m)) then
                ...
                if (q_dst(i,k,m) > q_dst_sgn) write (6,'(a,i2,a,i3,a,i2,a,i2,a,es8.1)')  &
                     'dead: pcp_fxr: lat = ',lat_idx,' q_dst_tnd_pcp(',i,',',k,',',m,') = ',q_dst_tnd_pcp(i,k,m)
       ...
       do i=1,plon
          ...
          if (pcp_flx_sfc(i) > 0.0_r8) then
             ...
             if (pcp_frc_evp > 1.0.or.pcp_frc_evp < 0.0_r8) write(6,'(a,i2,a,i3,a,i2,a,es8.1)')  &
                  'dead: pcp_evp: lat = ',lat_idx,' pcp_frc_evp(',i,',',k,') = ',pcp_frc_evp
#endif /* not DST_DBG */
          else
          ...
          do m=1,dst_nbr
             ...
             if (q_dst_tnd_evp(i,k,m) > flx_mss_pcp_sfc(i,m)/mpl_air(i,k)) then
                if (flx_mss_evp > flx_mss_sgn) write (6,'(a,i2,a,i3,a,i2,a,i2,a,es8.1,a,es8.1)')  &
                     'dead: evp_fxr: lat = ',lat_idx,' flx_mss_evp(',i,',',k,',',m,') = ',flx_mss_evp, &
  ...
  subroutine dpswet2nc(             &
    ...
    integer dmn_lon_lev_sz_time(4) ! [enm] Dimension IDs
    integer dmn_lon_levp_sz_time(4) ! [enm] Dimension IDs
    ...
    dmn_lon_lev_sz_time=(/lon_dmn_id,lev_dmn_id,sz_dmn_id,time_dmn_id/)
    ...
    dmn_lon_levp_sz_time=(/lon_dmn_id,levp_dmn_id,sz_dmn_id,time_dmn_id/)
    cnt_lon_levp_sz_time=(/plon,plevp,dst_nbr,1/)
    srt_lon_levp_sz_time=(/1,1,1,nstep/)
    
    if (nstep == 1) then
       ! Variable definitions
       rcd=rcd+nf90_def_var(nc_id,'cld_frc',nf90_float,dmn_lon_levp_time,cld_frc_id)
       rcd=rcd+nf90_def_var(nc_id,'cld_frc_cnv',nf90_float,dmn_lon_lev_time,cld_frc_cnv_id)
       rcd=rcd+nf90_def_var(nc_id,'cld_vlm',nf90_float,dmn_lon_lev_time,cld_vlm_id)
       rcd=rcd+nf90_def_var(nc_id,'flx_mss_pcp_sfc',nf90_float,dmn_lon_sz_time,flx_mss_pcp_sfc_id)
       rcd=rcd+nf90_def_var(nc_id,'flx_mss_pcp_sfc_ttl',nf90_float,dmn_lon_time,flx_mss_pcp_sfc_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'frc_trc_trn_cnv_ptn',nf90_float,dmn_lon_lev_sz_time,frc_trc_trn_cnv_ptn_id)
       rcd=rcd+nf90_def_var(nc_id,'pcp_flx_sfc',nf90_float,dmn_lon_time,pcp_flx_sfc_id)
       rcd=rcd+nf90_def_var(nc_id,'q_H2O_cnd',nf90_float,dmn_lon_lev_time,q_H2O_cnd_id)
       rcd=rcd+nf90_def_var(nc_id,'q_H2O_cnd2pcp_tnd',nf90_float,dmn_lon_lev_time,q_H2O_cnd2pcp_tnd_id)
       rcd=rcd+nf90_def_var(nc_id,'q_H2O_pcp2vpr_tnd',nf90_float,dmn_lon_lev_time,q_H2O_pcp2vpr_tnd_id)
       rcd=rcd+nf90_def_var(nc_id,'q_H2O_vpr2pcp_cnv_tnd',nf90_float,dmn_lon_lev_time,q_H2O_vpr2pcp_cnv_tnd_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst_tnd_evp',nf90_float,dmn_lon_lev_sz_time,q_dst_tnd_evp_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst_tnd_evp_ttl',nf90_float,dmn_lon_lev_time,q_dst_tnd_evp_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst_tnd_ncl',nf90_float,dmn_lon_lev_sz_time,q_dst_tnd_ncl_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst_tnd_pcp',nf90_float,dmn_lon_lev_sz_time,q_dst_tnd_pcp_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst_tnd_pcp_ttl',nf90_float,dmn_lon_lev_time,q_dst_tnd_pcp_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst_tnd_wet',nf90_float,dmn_lon_lev_sz_time,q_dst_tnd_wet_id)
       rcd=rcd+nf90_def_var(nc_id,'scv_cff_mss_avg_pcp_nrm_cnv',nf90_float,sz_dmn_id,scv_cff_mss_avg_pcp_nrm_cnv_id)
       rcd=rcd+nf90_def_var(nc_id,'scv_cff_mss_avg_pcp_nrm_str',nf90_float,sz_dmn_id,scv_cff_mss_avg_pcp_nrm_str_id)
       rcd=rcd+nf90_def_var(nc_id,'spc_xsx_ncl_scv',nf90_float,sz_dmn_id,spc_xsx_ncl_scv_id)

...
contains
  ...
  subroutine dst_mbl(lchnk,ncol,obuf, &
    ...
    use wbl_mdl,only:wbl_wnd,wnd_mdp_wbn_get ! [mdl] Weibull wind speed distribution
    implicit none
    ! Parameters
    ! Initialize resolution-independent debugging indices
    integer,parameter::wnd_dbg=1 ! [idx] Wind bin index for diagnostics
    integer,parameter::bln_dbg=1 ! [idx] Soil blend index for diagnostics
    integer,parameter::wnd_mdp_nbr=5 ! [nbr] Number of wind speed bins
#ifdef AlG01
    real(r8),parameter::flx_mss_fdg_fct=28.0 ! [frc] Global mass flux tuning factor (a posteriori)
#else
    real(r8),parameter::flx_mss_fdg_fct=7.0e-4 ! [frc] Global mass flux tuning factor (a posteriori)
#endif
    ...
    real(r8),parameter::wnd_frc_rsl=0.95_r8 ! [frc] Fraction of wind PDF to resolve
    ! Input
    ...
    real(r8),intent(in)::doy ! I [day] Day of year [1.0..367.0)
    ...
    integer wnd_mdp_idx ! [idx] Counting index for wind bin
    ...
    real(r8) mss_frc_CaCO3(plond) ! [kg kg-1] Mass fraction of CaCO3
    real(r8) lvl_dlt(plond)       ! [m] Soil layer thickness
    real(r8) mpl_air(plond)       ! [kg m-2] Air mass path in layer
    real(r8) mss_frc_cly(plond)   ! [kg kg-1] Mass fraction of clay
    real(r8) mss_frc_snd(plond)   ! [kg kg-1] Mass fraction of sand
    real(r8) sfc_frc_bln(plond,bln_nbr) ! [m2 m-2] Surface area fraction of soil blend
    real(r8) flx_mss_vrt_dst_rat_bln(plond,bln_nbr) ![frc] Fraction of vertical flux caused by a soil blend
    ...
    real(r8)::flx_mss_hrz_slt_ttl_wbn(plond) ! [kg m-1 s-1] Vertically integrated streamwise mass flux in wind bin
    real(r8)::flx_mss_vrt_dst_ttl_wbn(plond) ! [kg m-2 s-1] Vertical flux due to a wind bin
    real(r8)::mss_frc_src_add(plond,dst_src_nbr) ! [kg kg-1] Mass fraction source (added up)
    real(r8)::mss_frc_src_wbn(plond,dst_src_nbr) ! [kg kg-1] Mass fraction of source modes
    real(r8)::mss_frc_trn_dst_src_add(plond,dst_nbr) ! [kg kg-1] Mass fraction of source transported (added up)
    real(r8)::mss_frc_trn_dst_src_wbn(plond,dst_nbr) ! [kg kg-1] Mass fraction of source transported from bin
    real(r8)::ovr_src_snk_mss_add(plond,dst_src_nbr,dst_nbr) ! [frc] Overlap mass factor (added up)
    real(r8)::ovr_src_snk_mss_ttl_add(plond) ! [frc] Total overlap between source and sink (added up)
    real(r8)::ovr_src_snk_mss_ttl_wbn(plond) ! [frc] Total overlap between source and sink from bin
    real(r8)::ovr_src_snk_mss_wbn(plond,dst_src_nbr,dst_nbr) ! [frc] Overlap mass factor for bin
    real(r8)::wnd_mdp_max(plond) ! [m s-1] Maximum discretized wind speed
    real(r8)::wnd_mdp_min(plond) ! [m s-1] Minimum discretized wind speed
    real(r8)::wnd_mdp_wbn(plond) ! [m s-1] Nominal wind speed in bin
    real(r8)::wnd_mdp_wgt(plond,wnd_mdp_nbr) ! [frc] Wind speed bin weight
    ...
    ! Main Code
    CEWI_flt=sum(prs_sfc)+obuf(1) ! CEWI
    ...
    ! Sandblasting implementation overwrites mss_frc_src_wbn dynamically
    mss_frc_src_wbn(:,:)=mss_frc_src(:,:) ! [kg kg-1] Mass fraction of source modes
    ! Zero because it is updated and weighted
    mss_frc_src_add(:,:)=0.0_r8 ! [kg kg-1] Mass fraction source (added up)
    ! Initialize overlap of source and sink
    ovr_src_snk_mss_wbn(:,:,:)=ovr_src_snk_mss(:,:,:)
    ovr_src_snk_mss_add(:,:,:)=0.0_r8
    ! Initialize total overlap src/sink
    ovr_src_snk_mss_ttl_wbn(:)=ovr_src_snk_mss_ttl(:)
    ovr_src_snk_mss_ttl_add(:)=0.0_r8
    mss_frc_trn_dst_src_wbn(:,:)=mss_frc_trn_dst_src(:,:)
    mss_frc_trn_dst_src_add(:,:)=0.0_r8
    ...
    call soi_txt_get( &
         ...
         mss_frc_CaCO3,       & ! O [kg kg-1] Mass fraction of CaCO3
         mss_frc_cly,         & ! O [kg kg-1] Mass fraction of clay
         mss_frc_snd,         & ! O [kg kg-1] Mass fraction of sand
         sfc_frc_bln)         ! O [m2 m-2] Surface area fraction of soil blend
    ! Soil moisture
    call vwc_sfc_get( &
         lat_idx,             & ! I [idx] Latitude index
         plon,                & ! I [frc] Dimension size (normally plon)
    ...
    call hyd_prp_get(         & ! NB: These properties are time-invariant
         ...
         mss_frc_cly,         & ! I [kg kg-1] Mass fraction clay 
         mss_frc_snd,         & ! I [kg kg-1] Mass fraction sand
    ...
    ! Weighting factors and nominal wind speed distribution
    call wbl_wnd( &
    ...
    if (dbg_lvl > dbg_fl) then
       write(6,'(a)') "Wind PDF Diagnostics"
       write(6,'(a,2(f7.2,a))') "Mean horizontal wind speed at height = ",hgt_mdp(lon_dbg)," m is ",wnd_mdp(lon_dbg)," m s-1"
       write(6,'(a3,9(a8))') "Bin","wnd_mdp","wnd_frc","wnd_frcs","src1frc","src2frc","src3frc","flx_hrz","flx_vrt","wnd_wgt"
       write(6,'(a3,9(a8))') "idx","m s-1","cm s-1","cm s-1","frc","frc","frc","kg m-1 s-1","kg m-2 s-1","frc"
    endif ! endif dbg
#endif /* !DST_DBG */

    ! Loop over wind bins in PDF
    do wnd_mdp_idx=1,wnd_mdp_nbr

       if(wnd_mdp_nbr > 1) then ! More than one wind bin

          ! Determine nominal wind speed in bin
          call wnd_mdp_wbn_get( &
          ...
          call blm_mbl( &
               ...
               tpt_soi,             & ! I [K] Soil temperature
               trn_fsh_vpr_soi_atm, & ! I [frc] Transfer efficiency of vapor from soil to atmosphere
               wnd_mdp_wbn,         & ! I [m s-1] Nominal wind speed in bin
               mno_lng,             & ! O [m] Monin-Obukhov length
               tpt_gnd,             & ! I/O [K] Ground temperature
               wnd_frc)             ! O [m s-1] Surface friction velocity
       ...
       call frc_thr_ncr_wtr_get( &
            ...
            mss_frc_cly, & ! I [kg kg-1] Mass fraction of clay
       ...
       call flx_mss_hrz_slt_ttl_AlG01_get( &
            ...
            flx_mss_hrz_slt_ttl_lkt, & ! I [kg m-1 s-1] Saltation flux look-up table
            sfc_frc_bln, & ! I [m2 m-2] Surface area fraction of soil blend
...
#ifdef AlG01
       ! 20031010: Apply mass fraction of vertical flux caused by each soil blend
       ! flx_mss_vrt_dst_rat_bln is ratio of vertical flux to total vertical flux caused by each soil blend
       ! Weight mss_frc_src by flx_mss_vrt_dst_rat_bln
       ! sfc_frc_bln is surface fraction covered by soil blend 
       ! Until 20031010 we weighted mss_frc_src by sfc_frc_bln (which is nonsense)

       call flx_mss_vrt_dst_ttl_AlG01_get( &
            dst_slt_flx_rat_ttl_lkt, & ! I [frc] Look-up table for alpha (function of wind and soil type)
            dst_slt_flx_rat_ttl, & ! O [m-1] Ratio of vertical dust flux to streamwise mass flux
            flx_mss_hrz_slt_ttl_wbn, & ! I [kg m s-1] Vertically integrated streamwise mass flux
            flx_mss_hrz_slt_ttl_lkt, & ! I [kg m s-1] Look-up table for vertically integrated streamwise flux
            flx_mss_vrt_dst_ttl_wbn, & ! O [kg m-2 s-1] Total vertical mass flux of dust
            wnd_frc, & ! I [m s-1] Friction speed (fxm: use wnd_frc_slt instead?)
            sfc_frc_bln, & ! I [m2 m-2] Surface area fraction of soil blend
            flx_mss_vrt_dst_rat_bln) ! O [frc] Fraction of vertical flux caused by a soil blend
       
       !write(6,'(a10,4f10.2)')'surffrac',sfc_frc_bln(1,:)
       !write(6,'(a10,4f10.2)')'fluxfrac',flx_mss_vrt_dst_rat_bln(1,:)
       
       call mss_frc_src_AlG01_get( &
            wnd_frc, & ! I [m s-1] Friction velocity (fxm: use wnd_frc_slt instead?)
            mss_frc_src_lkt, & ! I [kg kg-1] Mass fraction source mode look-up table
            mss_frc_src_wbn, & ! O [kg kg-1] Mass fraction of source modes
            flx_mss_vrt_dst_rat_bln) ! I [frc] Fraction of vertical flux caused by a soil blend
            !sfc_frc_bln) ! I [frc] fraction of surface covered by a soil blend

       call ovr_src_snk_mss_AlG01_get( &
            mss_frc_src_wbn, & ! I [kg kg-1] Mass fraction of source modes
       ...
       do i=1,plon
#ifdef DST_DBG
          if (dbg_lvl > dbg_fl) then
             ! Print bin-specific fluxes before weights applied
             write(6,'(i2,1x,3(f8.2),6es8.1)') wnd_mdp_idx, &
                  wnd_mdp_wbn(i),wnd_frc(i)*100.0_r8,wnd_frc_slt(i)*100.0_r8, &
                  mss_frc_src_wbn(i,1),mss_frc_src_wbn(i,2),mss_frc_src_wbn(i,3), &
                  flx_mss_hrz_slt_ttl_wbn(i),flx_mss_vrt_dst_ttl_wbn(i),wnd_mdp_wgt(i,wnd_mdp_idx)
          endif ! endif dbg
#endif /* !DST_DBG */
          ...
          ! Add up horizontal saltation soil flux
    ...
    enddo ! end loop over wind bins in PDF

    ! Normalize total mass fractions
    do i=1,plon
       ! Output diagnostics for all wind speeds
       ...
       if (flx_mss_vrt_dst_ttl(i) > 0.0_r8) then
          ...
          ! Reset mass overlap factors when flux is zero
    ...
    if (flg_CaCO3) then
       call flx_mss_CaCO3_msk( &
            ...
            mss_frc_CaCO3,    & ! I [kg kg-1] Mass fraction of CaCO3
            mss_frc_cly,      & ! I [kg kg-1] Mass fraction of clay
            mss_frc_snd)      ! I [kg kg-1] Mass fraction of sand
...
#ifdef DST_DBG
             if (q_dst(i,m) > q_dst_mxm) write(6,'(a,f7.3,a,i2,a,i3,a,i2,a,es8.1,a)')  &
                  'dst_mbl: High concentration WARNING doy = ',doy,', lat = ',lat_idx, &
                  ' q_dst(',i,',',m,') = ',q_dst(i,m),' kg kg-1'
#endif /* !DST_DBG */
          end do              ! end loop over cst
#ifdef DST_DBG
          if (flx_mss_vrt_dst_ttl(i) > flx_mss_mxm) write(6,'(a,f7.3,a,i2,a,i3,a,es8.1,a)')  &
               'dst_mbl: High flux WARNING doy = ',doy,', lat = ',lat_idx, &
...
737 continue
    ...
    if (lat_idx == lat_dbg) then
       write (6,'(a,1(a,f9.5,a),19(a,es9.2,a))') &
            'dead: Diagnostics at (lat_dbg,lon_dbg) set in dst_dbg_cmn_ini() :', &
            'doy = ',doy,' day, ', &
    ...
    call mbl2nc(             &
         ...
         wnd_rfr,             & ! I [m s-1] Wind speed at reference height
         wnd_rfr_thr_slt,     & ! I [m s-1] Threshold 10 m wind speed for saltation
         mss_frc_src_add,         & ! I [kg kg-1] Mass fraction of source distribution 
         mss_frc_trn_dst_src_add, & ! I [frc] Fraction of transported dust mass at source
         ovr_src_snk_mss_add,     & ! I [frc] Mass overlap of src with snk 
  ...
  subroutine mbl2nc(             &
       ...
       wnd_rfr,             & ! I [m s-1] Wind speed at reference height
       wnd_rfr_thr_slt,     & ! I [m s-1] Threshold 10 m wind speed for saltation
       mss_frc_src,         & ! I [kg kg-1] Mass fraction of source distribution 
       mss_frc_trn_dst_src, & ! I [frc] Fraction of transported dust mass at source
       ovr_src_snk_mss,     & ! I [frc] Mass overlap of src with snk 
    ...
    real(r8) wnd_rfr(plond)       ! [m s-1] Wind speed at reference height
    real(r8) wnd_rfr_thr_slt(plond) ! [m s-1] Threshold 10 m wind speed for saltation
    real(r8) mss_frc_src(plond,dst_src_nbr) ! [kg kg-1] Mass fraction of source distribution
    real(r8) mss_frc_trn_dst_src(plond,dst_nbr) ! [frc] Fraction of transported dust mass at source
    real(r8) ovr_src_snk_mss(plond,dst_src_nbr,dst_nbr) ! [frc] Mass overlap of src with snk
    real(r8) ovr_src_snk_mss_ttl(plond) ! [frc] Total transported mass fraction of dust flux
    ! Output
    ! Local
    ! File metadata and dimension IDs
    integer dmn_lon_sz_src_sz_time(4) ! [enm] Dimension IDs
    integer dmn_lon_sz_src_time(3) ! [enm] Dimension IDs
    integer dmn_lon_sz_time(3) ! [enm] Dimension IDs
    ...
    dmn_lon_sz_time=(/lon_dmn_id,sz_dmn_id,time_dmn_id/)
    srt_lon_sz_time=(/1,1,nstep/)
    cnt_lon_sz_time=(/plon,dst_nbr,1/)

    dmn_lon_sz_src_sz_time=(/lon_dmn_id,sz_src_dmn_id,sz_dmn_id,time_dmn_id/)
    srt_lon_sz_src_sz_time=(/1,1,1,nstep/)
    cnt_lon_sz_src_sz_time=(/plon,dst_src_nbr,dst_nbr,1/)

    dmn_lon_sz_src_time=(/lon_dmn_id,sz_src_dmn_id,time_dmn_id/)
    srt_lon_sz_src_time=(/1,1,nstep/)
    cnt_lon_sz_src_time=(/plon,dst_src_nbr,1/)
    
    if (nstep == 1) then
       ! Variable definitions
       rcd=rcd+nf90_def_var(nc_id,'frc_thr_ncr_drg',nf90_float,dmn_lon_time,frc_thr_ncr_drg_id)
       rcd=rcd+nf90_def_var(nc_id,'dst_slt_flx_rat_ttl',nf90_float,dmn_lon_time,dst_slt_flx_rat_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'flx_mss_hrz_slt_ttl',nf90_float,dmn_lon_time,flx_mss_hrz_slt_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'flx_mss_vrt_dst',nf90_float,dmn_lon_sz_time,flx_mss_vrt_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'flx_mss_vrt_dst_ttl',nf90_float,dmn_lon_time,flx_mss_vrt_dst_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'frc_thr_ncr_wtr',nf90_float,dmn_lon_time,frc_thr_ncr_wtr_id)
       rcd=rcd+nf90_def_var(nc_id,'gwc_sfc',nf90_float,dmn_lon_time,gwc_sfc_id)
       rcd=rcd+nf90_def_var(nc_id,'hgt_zpd_mbl',nf90_float,hgt_zpd_id)
       rcd=rcd+nf90_def_var(nc_id,'lnd_frc_mbl',nf90_float,dmn_lon_time,lnd_frc_mbl_id)
       rcd=rcd+nf90_def_var(nc_id,'mno_lng_mbl',nf90_float,dmn_lon_time,mno_lng_id)
       rcd=rcd+nf90_def_var(nc_id,'rgh_mmn_mbl',nf90_float,rgh_mmn_id)
       rcd=rcd+nf90_def_var(nc_id,'snw_frc',nf90_float,dmn_lon_time,snw_frc_id)
       rcd=rcd+nf90_def_var(nc_id,'vai_dst',nf90_float,dmn_lon_time,vai_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'wnd_frc_mbl',nf90_float,dmn_lon_time,wnd_frc_id)
       rcd=rcd+nf90_def_var(nc_id,'wnd_frc_slt',nf90_float,dmn_lon_time,wnd_frc_slt_id)
       rcd=rcd+nf90_def_var(nc_id,'wnd_frc_thr_slt',nf90_float,dmn_lon_time,wnd_frc_thr_slt_id)
       rcd=rcd+nf90_def_var(nc_id,'wnd_mdp',nf90_float,dmn_lon_time,wnd_mdp_id)
       rcd=rcd+nf90_def_var(nc_id,'wnd_rfr_mbl',nf90_float,dmn_lon_time,wnd_rfr_id)
       rcd=rcd+nf90_def_var(nc_id,'wnd_rfr_thr_slt',nf90_float,dmn_lon_time,wnd_rfr_thr_slt_id)
       rcd=rcd+nf90_def_var(nc_id,'mss_frc_src',nf90_float,dmn_lon_sz_src_time,mss_frc_src_id)
       rcd=rcd+nf90_def_var(nc_id,'mss_frc_trn_dst_src',nf90_float,dmn_lon_sz_time,mss_frc_trn_dst_src_id)
       rcd=rcd+nf90_def_var(nc_id,'ovr_src_snk_mss',nf90_float,dmn_lon_sz_src_sz_time,ovr_src_snk_mss_id)
       rcd=rcd+nf90_def_var(nc_id,'ovr_src_snk_mss_ttl',nf90_float,dmn_lon_time,ovr_src_snk_mss_ttl_id)

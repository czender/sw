...
contains
  ...
  subroutine wnd_frc_thr_slt_get( &
    ...
    ! Main Code
    CEWI_flt=sum(dmt_aer)+sum(dns_aer) ! CEWI
  ...
  subroutine frc_thr_ncr_wtr_get( &
    ...
    do lon_idx=1,plon
       if (flg_mbl(lon_idx)) then
          ...
          ! gwc_thr=mss_frc_cly*(0.17_r8+0.14_r8*mss_frc_cly) ! [kg kg-1] FMB99 p. 155 (14)
          ! fxm: 19991105 remove factor of mss_frc_cly from gwc_thr to improve large scale behavior
          gwc_thr(lon_idx)=0.17_r8+0.14_r8*mss_frc_cly(lon_idx) ! [kg kg-1] 
  ...
  subroutine mss_frc_src_AlG01_get(&
       ...
       mss_frc_src_lkt, & ! I [frc] Mass fraction source look-up table
       mss_frc_src, & ! O [frc] Mass fraction of source mode
       flx_mss_vrt_dst_rat_bln) ! I [frc] Fraction of vertical flux caused by each soil blend
    ! Purpose: Find mass fraction of each source mode in vertical mass flux
    ! Source modes of Alfaro & Gomes, JGR, 2001 (AlG01)
    ...
    ! Input
    real(r8),intent(in)::mss_frc_src_lkt(wnd_frc_nbr,bln_nbr,dst_src_nbr) ! I [frc] Look-up table for mss_frc_src
    real(r8),intent(in)::wnd_frc(plond) ! I [m s-1] Wind friction speed
    real(r8),intent(in)::flx_mss_vrt_dst_rat_bln(plond,bln_nbr)! [frc] Fraction of vertical mass flux caused by each soil blend
    ! Local
    integer::lon_idx ! [idx] counting variable for longitude
    integer::src_idx ! [idx] counting variable for source mode
    integer::bln_idx ! [idx] counting variable for soil type
    integer::wnd_frc_idx ! [idx] Nearest integer [1..100] to wind friction speed [cm s-1]
    ...
    do src_idx=1,dst_src_nbr
       do bln_idx=1,bln_nbr
          do lon_idx=1,plon
             ! [idx] Nearest integer [1..100] to wind friction speed [cm s-1] at this longitude
             wnd_frc_idx=min(100,int(100.0_r8*wnd_frc(lon_idx)+0.5_r8)) ! [idx]
             
             ! Fraction of sandblasting from each sandblasted mode is...
             mss_frc_src(lon_idx,src_idx) = & 
                  ! ...contribution from sandblasting by preceding saltator populations plus...
                  mss_frc_src(lon_idx,src_idx)+ & ! mss_frc_src is three first elements
                  ! ...contribution to this sandblasted mode by this saltator population...
                  flx_mss_vrt_dst_rat_bln(lon_idx,bln_idx)* &
                  mss_frc_src_lkt(wnd_frc_idx,bln_idx,src_idx)
          enddo ! Loop on lon
  ...
  subroutine ovr_src_snk_mss_AlG01_get( &
    ...
    ! Purpose: Compute overlap between source and sink based on mass fraction of each source mode.
    ! Code by Alf Grini, UCI/UiO, 2002
  ...
  subroutine flx_mss_vrt_dst_ttl_AlG01_get( &
       ...
       flx_mss_hrz_slt_ttl_lkt, & ! I [kg m s-1] Look-up table for horizontal dust flux
       flx_mss_vrt_dst_ttl, & ! O [kg m-2 s-1] total vertical dust flux
       wnd_frc, & ! I [m s-1] wind friction speed
       sfc_frc_bln, & ! I [frc] fraction surface covered by a soil blend
       flx_mss_vrt_dst_rat_bln) ! O [frc] fraction of vertical flux caused by each soil blend
    ...
    real(r8),intent(out)::flx_mss_vrt_dst_rat_bln(plond,bln_nbr) ! O [frc] fraction of vertical mass flux
    ! caused by a soil blend
    ! Input
    real(r8),intent(in)::flx_mss_hrz_slt_ttl(plond) ! I [kg m-1 s-1] total horizontal mass flux of soil
    real(r8),intent(in)::flx_mss_hrz_slt_ttl_lkt(wnd_frc_nbr,bln_nbr) ! I [kg m-1 s-1] LUT for horizontal mass flux of soil
    real(r8),intent(in)::dst_slt_flx_rat_ttl_lkt(wnd_frc_nbr,bln_nbr) ! I [m-1] ratio of vertical to horizontal flux
    real(r8),intent(in)::wnd_frc(plond) ! I [m s-1] wind friction speed
    real(r8),intent(in)::sfc_frc_bln(plond,bln_nbr) ! [frc] fractin of surface covered by a soil blend
    ! Local
    integer::bln_idx ! [idx] index for soil type
    integer::wnd_frc_idx ! [idx] Nearest integer [1..100] to wind friction speed [cm s-1]
    integer::lon_idx ! [idx] index for longitude
    real(r8)::flx_mss_vrt_dst_ttl_unt ! [kg m-2 s-1] total vertical flux (untuned)
    ! Initialize
    ...
    do bln_idx=1,bln_nbr
       do lon_idx=1,plon
          ! Nearest integer [1..100] to wind friction speed in [cm s-1] at this longitude
          wnd_frc_idx=min(100,int(100.0_r8*wnd_frc(lon_idx)+0.5_r8)) ! [idx]
          
          ! This "untuned vertical flux" is acutal vertical flux without fudge factor
          ! At this point, flx_mss_vrt_dst_rat_bln contains a vertical flux [kg m-2 s-1] 
          ! This is currently total vertical flux sandblasted from a soil mode
          ! flx_mss_vrt_dst_rat_bln is converted to a fraction later
          ! "untuned vertical flux" due to each saltator population is...
          flx_mss_vrt_dst_rat_bln(lon_idx,bln_idx)= &
               ! ...surface fraction of saltator population times...
               sfc_frc_bln(lon_idx,bln_idx)* &
               ! ...untuned horizontal flux due to saltator population times...
               flx_mss_hrz_slt_ttl_lkt(wnd_frc_idx,bln_idx)* &
               ! ...mass sandblasting efficiency of population...
               dst_slt_flx_rat_ttl_lkt(wnd_frc_idx,bln_idx)
          
       enddo ! end loop over longitude
    enddo ! end loop over soil blends
    
    ! Determine mass sandblasting efficiency "alpha" [m-1] from above calculation
    ! We know vertical flux caused by each soil blend without fudge factor
    ! Divide this by total horizontal flux without fudge factor
    do lon_idx=1,plon
       ! Nearest integer [1..100] to wind friction speed [cm s-1] at this longitude
       wnd_frc_idx=min(100,int(100.0_r8*wnd_frc(lon_idx)+0.5_r8)) ! [idx]
       
       ! Is there any horizontal flux (saltation)?
       if(sum(flx_mss_hrz_slt_ttl_lkt(wnd_frc_idx,:)) > 0.0_r8) then
          ! Recall, flx_mss_vrt_dst_rat_bln is still a flux [kg m-2 s-1] for now
          ! Mass sandblasting efficiency "alpha" at a point is ...
          dst_slt_flx_rat_ttl(lon_idx)= &
               ! ...ratio of total sandblasted flux by all saltator populations...
               sum(flx_mss_vrt_dst_rat_bln(lon_idx,:))/ &
               ! ...to total saltator flux of all saltator populations...
               sum(sfc_frc_bln(lon_idx,:)*flx_mss_hrz_slt_ttl_lkt(wnd_frc_idx,:))
       else ! No horizontal flux
          dst_slt_flx_rat_ttl(lon_idx)=0.0_r8
       endif
    enddo ! end loop over longitude 
    
    ! Convert vertical (untuned) fluxes into ratio of vertical flux caused by a soil blend
    do lon_idx=1,plon
       ! Is there any vertical flux (sandblasting)?
       if(sum(flx_mss_vrt_dst_rat_bln(lon_idx,:)) > 0.0_r8)then
          ! Get untuned vertical flux
          flx_mss_vrt_dst_ttl_unt=sum(flx_mss_vrt_dst_rat_bln(lon_idx,:))
          ! Mass fraction of vertical flux from saltation-sandblasting of each soil blend
          ! Until now flx_mss_vrt_dst_rat_bln has been a flux [kg m-2 s-1]
          flx_mss_vrt_dst_rat_bln(lon_idx,:)= &
               flx_mss_vrt_dst_rat_bln(lon_idx,:)/flx_mss_vrt_dst_ttl_unt
       else ! No sandblasting
          flx_mss_vrt_dst_rat_bln(lon_idx,:)=0.0_r8
       endif ! No sandblasting
    enddo ! Loop on longitude
    
    ! Total vertical flux is based on tuned horizontal flux
    ! Apply value of dst_slt_flx_rat_ttl calculated above to tuned horizontal flux
    do lon_idx=1,plon
       flx_mss_vrt_dst_ttl(lon_idx)= & ! [kg m-2 s-1] Total vertical dust flux
            flx_mss_hrz_slt_ttl(lon_idx)* & ! [kg m-1 s-1] Total horizontal flux 
            dst_slt_flx_rat_ttl(lon_idx) ! [m-1] Total ratio of vertical to horizontal flux
    enddo
    return
  ...
  subroutine flx_mss_hrz_slt_ttl_AlG01_get( &
       ...
       flx_mss_hrz_slt_ttl_lkt, & ! I [kg m-1 s-1] Look-up table for horizontal flux
       sfc_frc_bln, & ! I [frc] Look-up table for soil blends
       wnd_frc) ! I [m s-1] wind friction speed
    ! Purpose: Look up "complex" horizontal saltation flux 
    ! This complex flux is calculated offline in sltsbl module, accessible by cvs at 
    ! dust.ess.uci.edu:/home/zender/cvs/sltsbl
    ! Complex flux takes into account real microphysics of saltation where different
    ! soil sizes interact with wind friction speed and soil size distributions.
    ! Look-up tables are initialized in dst_slt_sbl_cmn_ini()
    ! Author: Alf Grini: alf.grini@geofysikk.uio.no
    use shr_kind_mod
    use pmgrid,only:plon,plond
    use dstgrd,only:bln_nbr,wnd_frc_nbr
    implicit none
    ! Input
    real(r8),intent(in)::flx_mss_hrz_slt_ttl_lkt(wnd_frc_nbr,bln_nbr) ! [kg m-1 s-1]look-up table for horizontal flux
    ...
    do bln_idx=1,bln_nbr
       do i=1,plon
          ! Nearest integer [1..100] to wind friction speed [cm s-1] at this longitude
          wnd_frc_idx=min(100,int(100.0_r8*wnd_frc(i)+0.5_r8)) ! [idx]
          
          ! Total horizontal saltation flux equals flux of previous blends plus...
          flx_mss_hrz_slt_ttl(i)=flx_mss_hrz_slt_ttl(i)+ &
               ! ...horizontal saltation flux of this blend weighted by ...
               flx_mss_hrz_slt_ttl_lkt(wnd_frc_idx,bln_idx)* & 
               ! ...surface fraction of this blend
               sfc_frc_bln(i,bln_idx)
       enddo
    enddo
  ...
  subroutine flx_mss_vrt_dst_prt( &
    ...
    ! Theory: 
    ! 20030128: Use overlap fraction calculated interactively in dstmbl() rather than 
    ! overlap fraction prescribed in dstaer module
    use dstgrd ! [mdl] Dust grid sizes
    use pmgrid ! [mdl] Spatial resolution parameters
    implicit none
    ! Input
  ...
  subroutine vai_lsm_get( & ! [fnc] Update time-varying surface fields
    ...
    ! vai_lsm_get() is not currently called because LSM VAI is read from dust boundary data
    ! If LSM surface type were truly prognostic (i.e., variations were not prescribed seasonally), 
    ! then it would make sense for vai_lsm_get() to provide vai_lsm.
    ! In that case, vai_lsm_get() would be called by CCM:dynamics/advnce(), MATCH:src/main()
    ! NB: routine currently returns leaf area index, not total vegetation area index = leaf + stem
    use pmgrid ! [mdl] Spatial resolution parameters
    ...
    real(r8),intent(in)::doy ! [day] Day of year [1.0..367.0)
    ! Output
    ...
    do lat_idx=1,plat
       do lon_idx=1,plon
          ...
          if ((sfc_dst_mbl_frc(lon_idx,lat_idx) > 1.0_r8).or. &
             ...
             write(6,'(a,i3,a,i2,a,f6.3)') &
                  'dead: dst_lsm_ini(): sfc_dst_mbl_frc(', &
                  lon_idx,',',lat_idx,') = ', &
                  sfc_dst_mbl_frc(lon_idx,lat_idx)
  ...
  subroutine tm_2_idx_wgt( & ! [fnc] Day of year --> bounding indices, weights
    ...
    day_grow=real(iday_NH) ! [day] Days since Jan 1 in NH or Jul 1 in SH (1.0..365.0)
    ...
    if (flg_SH_adj) then
       if (lat_dgr < 0.0_r8) then
          ...
          day_grow=real(iday_SH) ! [day] Days since Jan 1 in NH or Jul 1 in SH (1.0..365.0)
       endif ! endif flg_SH_adj
    end if ! endif
    mth_grow=12.0_r8*(day_grow-0.5_r8)/real(dpy) ! [mth] Months since Jan (NH) or Jul (SH) (1.0..12.0)
    ...
    idx_mth_glb=int(mth_grow+0.5_r8) ! [idx] Interpolation month, future
    idx_mth_lub=idx_mth_glb+1 ! [idx] Interpolation month, past
    wgt_glb=(real(idx_mth_glb)+0.5_r8)-mth_grow ! [frc] Interpolation weight
  ...
  subroutine lnd_frc_mbl_get( &
    ...
    lnd_nbr=0
    do lon_idx=1,plon
       lnd_idx(lon_idx)=-73 ! Initialize to dummy value

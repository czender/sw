...
! 20020316: Switch from She84 to Dal87 as default
  ...
  subroutine dst_psd_ini() ! [fnc] Initialize particle size distributions
    ...
    real(r8),dimension(9),parameter::dmt_grd_08= & ! WRF-Chemistry Grid
         (/ 0.0390625e-6,0.078125e-6,0.15625e-6,0.3125e-6,0.625e-6,1.25e-6,2.5e-6,5.0e-6,10.0e-6/) ! [m] Particle diameter grid
    ...
    write (6,'(a,f7.4)') 'asp_rat_lps_dfl = ',asp_rat_lps_dfl
    ...
    if (dst_nbr == 8) then
       do idx=1,dst_nbr+1     ! NB: dst_nbr+1
          dmt_grd(idx)=dmt_grd_08(idx) ! [m] Particle diameter grid
       end do                  ! end loop over grd
       do idx=1,dst_nbr
          dmt_min(idx)=dmt_grd_08(idx) ! [m] Maximum diameter in bin
          dmt_max(idx)=dmt_grd_08(idx+1) ! [m] Minimum diameter in bin
          dmt_ctr(idx)=0.5_r8*(dmt_min(idx)+dmt_max(idx)) ! [m] Diameter at bin center
          dmt_dlt(idx)=dmt_max(idx)-dmt_min(idx) ! [m] Width of size bin
       end do                  ! end loop over grd
    endif                     ! endif dst_nbr == 4
    ...
    ! absolute mass fraction mapping from source modes to sink bins
    ! \sum_{i=1}^{i=src_nbr} \sum_{j=1}^{j=snk_nbr} ovr_src_snk_mss(i,j) <= 1
    ! with equality holding iff sources and sinks completely overlap
    ! Sum of ovr_src_snk_mss(:,:) is fraction of vertical dust flux which is transported
    ovr_src_snk_mss_ttl(:)=0.0_r8   ! [frc] Total transported mass fraction of dust flux
    mss_frc_trn_dst_src(:,:)=0.0_r8 ! [frc] Fraction of transported dust mass at source
    do snk_idx=1,dst_nbr
       do src_idx=1,dst_src_nbr
          do lon_idx=1,plon
          ovr_src_snk_mss(lon_idx,src_idx,snk_idx)= & ! [frc] Mass overlap of src with snk
   ...
   ! Convert fraction of mobilized mass to fraction of transported mass
    ...
    write (6,'(a,f6.3)') 'Total transported mass fraction of dust flux = ',ovr_src_snk_mss_ttl
  ...
  subroutine dst_sz_rsl( &
    ...
    ! Main Code
    CEWI_flt=0.0_r8*asp_rat_lps ! CEWI
    ! Initialize
  ...
  subroutine grd_mk(sz_grd_typ, & ! I
    ...
    if (sz_grd_typ == grd_typ_lnr) then
       sz_grd_rsn=(sz_grd_max-sz_grd_min)/real(sz_nbr,r8)
       do idx=1,sz_nbr
          sz_min(idx)=sz_grd_min+real(idx-1,r8)*sz_grd_rsn ! [m]
       end do                  ! end loop over grd
  ...
  subroutine aersz2nc()
    ...
    integer levp_dmn_id       ! [enm] Dimension ID for levp
    ...
    integer sz_grd_dmn_id     ! [enm] Dimension ID for sz grid
    ...
    ! netCDF4 
    integer::dfl_lvl=1 ! [enm] Deflate level
    ...
    rcd=rcd+nf90_def_dim(nc_id,'sz_src',dst_src_nbr,sz_src_dmn_id)
    rcd=rcd+nf90_def_dim(nc_id,'sz',dst_nbr,sz_dmn_id)
    rcd=rcd+nf90_def_dim(nc_id,'lat',plat,lat_dmn_id)
    rcd=rcd+nf90_def_dim(nc_id,'lon',plon,lon_dmn_id)
    rcd=rcd+nf90_def_dim(nc_id,'lev',plev,lev_dmn_id)
    rcd=rcd+nf90_def_dim(nc_id,'levp',plevp,levp_dmn_id)
    rcd=rcd+nf90_def_dim(nc_id,'sz_grd',dst_nbr+1,sz_grd_dmn_id)
    ! Assemble ID and count vectors for each multidimensional combination of dimensions
    dmn_lon_sz(1)=lon_dmn_id
    dmn_lon_sz(2)=sz_dmn_id
    dmn_lon_lev_sz(1)=lon_dmn_id
    dmn_lon_lev_sz(2)=lev_dmn_id
    dmn_lon_lev_sz(3)=sz_dmn_id
    dmn_sz_src_sz(1)=sz_src_dmn_id
    dmn_sz_src_sz(2)=sz_dmn_id
    ! Variable definitions
    rcd=rcd+nf90_def_var(nc_id,'asp_rat_lps',nf90_float,sz_dmn_id,asp_rat_lps_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_max',nf90_float,sz_dmn_id,dmt_max_id)
#ifdef ENABLE_NETCDF4
    ! Set HDF Lempel-Ziv compression level, if requested
    if (dfl_lvl > 0) rcd=rcd+nf90_def_var_deflate(nc_id,dmt_max_id,flg_shf,flg_dfl,dfl_lvl)
#endif /* !ENABLE_NETCDF4 */
    rcd=rcd+nf90_def_var(nc_id,'dmt_min',nf90_float,sz_dmn_id,dmt_min_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_mjr',nf90_float,sz_dmn_id,dmt_mjr_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_mnr',nf90_float,sz_dmn_id,dmt_mnr_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_naa',nf90_float,sz_dmn_id,dmt_naa_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_nma',nf90_float,sz_dmn_id,dmt_nma_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_nmr',nf90_float,sz_dmn_id,dmt_nmr_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_nwr',nf90_float,sz_dmn_id,dmt_nwr_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_sma',nf90_float,sz_dmn_id,dmt_sma_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_smr',nf90_float,sz_dmn_id,dmt_smr_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_swa',nf90_float,sz_dmn_id,dmt_swa_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_swr',nf90_float,sz_dmn_id,dmt_swr_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_vma',nf90_float,sz_dmn_id,dmt_vma_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_vma_src',nf90_float,sz_src_dmn_id,dmt_vma_src_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_vmr',nf90_float,sz_dmn_id,dmt_vmr_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_vwa',nf90_float,sz_dmn_id,dmt_vwa_id)
    rcd=rcd+nf90_def_var(nc_id,'dmt_vwr',nf90_float,sz_dmn_id,dmt_vwr_id)
    rcd=rcd+nf90_def_var(nc_id,'dns_aer',nf90_float,sz_dmn_id,dns_aer_id)
    rcd=rcd+nf90_def_var(nc_id,'gsd_anl',nf90_float,sz_dmn_id,gsd_anl_id)
    rcd=rcd+nf90_def_var(nc_id,'gsd_anl_src',nf90_float,sz_src_dmn_id,gsd_anl_src_id)
    rcd=rcd+nf90_def_var(nc_id,'nbr_spc_rsl',nf90_float,sz_dmn_id,nbr_spc_rsl_id)
    rcd=rcd+nf90_def_var(nc_id,'ovr_src_snk_frc',nf90_float,dmn_sz_src_sz,ovr_src_snk_frc_id)
    rcd=rcd+nf90_def_var(nc_id,'sfc_spc_rsl',nf90_float,sz_dmn_id,sfc_spc_rsl_id)
    rcd=rcd+nf90_def_var(nc_id,'sz',nf90_float,sz_dmn_id,sz_id)
    rcd=rcd+nf90_def_var(nc_id,'sz_grd',nf90_float,sz_grd_dmn_id,sz_grd_id)
    rcd=rcd+nf90_def_var(nc_id,'sz_src',nf90_float,sz_src_dmn_id,sz_src_id)
    rcd=rcd+nf90_def_var(nc_id,'vlm_spc_rsl',nf90_float,sz_dmn_id,vlm_spc_rsl_id)

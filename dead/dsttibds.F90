...
#include <latlon.h>
#endif /* not SCCM */
    ! Parameters
    character(len=*),parameter::sbr_nm='dst_tibds_ini' ! [sng] Subroutine name
    ...
    write(6,"(a29,1x,a)") "dead: Opened netCDF input file",fl_in
    ! Get dimension IDs
    ...
    if (rcd /= nf90_noerr) call nf90_err_exit(rcd,sbr_nm//":get_var lat")
    rcd=nf90_wrp(nf90_get_var(nc_id,lon_id,lon),"get_var lon")
    if (rcd /= nf90_noerr) call nf90_err_exit(rcd,sbr_nm//":get_var lon")
    ...
    rcd=nf90_wrp(nf90_get_var(nc_id,lnd_frc_dry_id,lnd_frc_dry,start=srt_lon_lat,count=cnt_lon_lat),"gv lnd_frc_dry")
    rcd=nf90_wrp(nf90_get_var(nc_id,mbl_bsn_fct_id,mbl_bsn_fct,start=srt_lon_lat,count=cnt_lon_lat),"gv mbl_bsn_fct")
    rcd=nf90_wrp(nf90_get_var(nc_id,mss_frc_CaCO3_id,mss_frc_CaCO3,start=srt_lon_lat,count=cnt_lon_lat),"gv mss_frc_CaCO3")
    rcd=nf90_wrp(nf90_get_var(nc_id,mss_frc_snd_id,mss_frc_snd,start=srt_lon_lat,count=cnt_lon_lat),"gv mss_frc_snd")
    rcd=nf90_wrp(nf90_get_var(nc_id,mss_frc_cly_id,mss_frc_cly,start=srt_lon_lat,count=cnt_lon_lat),"gv mss_frc_cly")
    rcd=nf90_wrp(nf90_get_var(nc_id,sfc_frc_bln_id,sfc_frc_bln,start=srt_lon_lat_bln,count=cnt_lon_lat_bln),"gv sfc_frc_bln")
    rcd=nf90_wrp(nf90_get_var(nc_id,sfc_typ_id,sfc_typ,start=srt_lon_lat,count=cnt_lon_lat),"gv sfc_typ")
    ! Close file

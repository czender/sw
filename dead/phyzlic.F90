...
module phyzlic ! [mdl] Physics driver
  ...
  private::clm2nc ! [sbr] Output aerosol column properties
...
! Compile real phys_drv() only for box model, else use dummy stub...
  ...
  subroutine phys_drv( & ! [sbr] Physics driver
    ...
    ! Set tracer to dust dry mass mixing ratio on entering physics loop
    ...
    ! Set tracer to dust dry mass mixing ratio before exiting physics loop
    q_dst(:,:,:)=q_cst(:,:,dst_idx_srt:dst_idx_end) ! I [kg kg-1] Dust mixing ratio
  ...
  subroutine clm2nc( & ! [sbr] Output aerosol column properties
    ...
    ! Variable data
    rcd=sum(int(dst_trn_ttl))+lat_idx ! CEWI
    ...
    if (nstep == 1) then
       ...
       rcd=rcd+nf90_def_var(nc_id,'mpc_dst',nf90_float,dmn_lon_sz_time,mpc_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'mpc_dst_ttl',nf90_float,dmn_lon_time,mpc_dst_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'odxc_dst',nf90_float,dmn_lon_sz_time,odxc_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'odxc_dst_ttl',nf90_float,dmn_lon_time,odxc_dst_ttl_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst',nf90_float,dmn_lon_lev_sz_time,q_dst_id)
       rcd=rcd+nf90_def_var(nc_id,'q_dst_ttl',nf90_float,dmn_lon_lev_time,q_dst_ttl_id)

...
contains
  ...
  subroutine lgn_evl(abc_nbr,abc_ctr,mdn,gsd, & ! I
    ...
    real(DBLKIND) ln_gsd      ! [frc] ln(gsd)
    ...
    ln_gsd=log(real(gsd,DBLKIND))
    lngsdsqrttwopi_rcp=1.0_DBLKIND/(ln_gsd*sqrt(2.0_DBLKIND*pi))
    do idx=1,abc_nbr
       tmp=log(real(abc_ctr(idx)/mdn,DBLKIND))/ln_gsd
       xpn=exp(-0.5_DBLKIND*tmp*tmp)
       lgn_dst(idx)=real(lngsdsqrttwopi_rcp*xpn/real(abc_ctr(idx),DBLKIND),r8)

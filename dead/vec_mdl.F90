...
100    continue               ! Branch from identity interpolation
  ...
  subroutine rbn_vec(in_nbr,grd_in,dat_in, &
    ...
    do in_idx=1,in_nbr
       crd_in(in_idx)=0.5_r8*(grd_in_mnt(in_idx)+grd_in_mnt(in_idx+1))
    end do                     ! end loop over input coordinates
    do out_idx=1,out_nbr
       crd_out(out_idx)=0.5_r8*(grd_out_mnt(out_idx)+grd_out_mnt(out_idx+1))
    end do                     ! end loop over output coordinates
       ...
       if (brk_lft_idx(out_idx) < 0) then
          ...
          if (brk_lft_idx(out_idxp1) == 0) then
             ...
             if (.not.xtr_flg_LHS(xtr_prt_msk)) then
                ...
                trp_area_rgt=0.5_r8*crd_dlt_rgt*(dat_ntp_out(out_idxp1)+dat_ntp_in(brk_lft_idx(out_idxp1)))
          ...
          if (trp_nbr < 0) then
             ...
             dat_out(out_idx)=0.5_r8*(dat_ntp_out(out_idx)+dat_ntp_out(out_idxp1))
             ...
             trp_area_lft=0.5_r8*crd_dlt_lft*(dat_ntp_out(out_idx)+dat_ntp_in(brk_rgt_idx(out_idx)))
             crd_dlt_rgt=grd_out_mnt(out_idxp1)-grd_in_mnt(brk_lft_idx(out_idxp1))
             trp_area_rgt=0.5_r8*crd_dlt_rgt*(dat_ntp_out(out_idxp1)+dat_ntp_in(brk_lft_idx(out_idxp1)))
    ...
    if (dbg_lvl >= dbg_io) then
       ...
       do out_idx=1,out_nbr
          ...
          do trp_idx=1,trp_nbr-1
             in_idx=trp_srt_idx+trp_idx
             write (6,'(i4,a1)',advance="no") in_idx,','
          end do              ! end loop over trp
          in_idx=trp_srt_idx+trp_nbr
          if (trp_nbr > 0) then 
             ! fxm 20040907: valgrind consistently reports memory warnings here
             ! "Conditional jump or move depends on uninitialised value(s)"
             write (6,'(i4)') trp_srt_idx+trp_nbr 
          else 
             write (6,'(a4)') 'none'
          endif               ! endif
       end do                 ! end loop over crd_out

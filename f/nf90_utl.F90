...
module nf90_utl ! [mdl] netCDF utilities
  ...
  integer,parameter::nco_format_undefined=0 ! [enm] Output file format
...
contains
  ...
  integer function nf90_wrp_create(fl_nm,mode,nc_id,sbr_nm)  
    ...
    ! Main code
    rcd=nf90_create(fl_nm,mode,nc_id) ! [rcd] nf90_noerr == 0
    nf90_wrp_create=rcd  
  ...
  integer function nf90_wrp_open(fl_nm,mode,nc_id,sbr_nm)  
    ...
    integer::rcd ! [rcd] Return success code 
    ! Main code
    rcd=nf90_open(fl_nm,mode,nc_id) ! [rcd] nf90_noerr == 0
    nf90_wrp_open=rcd  
    if (rcd /= nf90_noerr) then  
       if(present(sbr_nm)) then 
  ...
  integer function nf90_wrp_close(nc_id,fl_nm,vrb_sng,sbr_nm)
    ...
    integer::rcd ! [rcd] Return success code 
    ! Main code
    rcd=nf90_close(nc_id) ! [rcd] nf90_noerr == 0
    nf90_wrp_close=rcd  
    if (rcd /= nf90_noerr) then

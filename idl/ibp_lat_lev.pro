...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Begin lat_lev() event handler
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro lat_lev_event,event
...
if (usr_value eq 'Wlat_lev_gph') then begin
...
endif; endif Wlat_lev_gph

if command_sng ne '' then print,command_sng

end; end lat_lev() event handler
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; End lat_lev() event handler
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
widget_xst=1
if n_elements(Wlat_lev_gph) ne 0 then begin
	widget_xst=widget_info(Wlat_lev_gph,/valid_id)
endif

if ((widget_xst) and (n_elements(Wlat_lev_gph) ne 0)) then begin
curr_sz=indgen(2)
widget_control,Wlat_lev_base,tlb_get_size=curr_sz

; If the window has been resized, then destroy the widget hierarchy
; so that the widget can be recreated at the new size.
wset,lat_lev_wdw

if ((lat_lev_wdw_x_sz ne curr_sz(0)) or (lat_lev_wdw_y_sz ne curr_sz(1))) then begin
	lat_lev_wdw_x_sz=curr_sz(0)
	lat_lev_wdw_y_sz=curr_sz(1)
	widget_control,Wlat_lev_base,/destroy
	widget_xst=0
endif

endif

if ((widget_xst ne 1) or (n_elements(Wlat_lev_gph) eq 0)) then begin

; Create the Latitude Level widget hierarchy
Wlat_lev_base=widget_base( $
	group_leader=Wtop_lvl, $
	title='Latitude Level')
Wlat_lev_gph=widget_draw( $
	Wlat_lev_base, $
		xsize=lat_lev_wdw_x_sz, $
		ysize=lat_lev_wdw_y_sz, $
	/button_events, $
	uvalue='Wlat_lev_gph')

widget_control,Wlat_lev_base,/realize
widget_control,get_value=lat_lev_wdw,Wlat_lev_gph
...
xmanager, $
	...
	Wlat_lev_base, $
	group_leader=Wtop_lvl, $
	event_handler='lat_lev_event'		

endif; end creating widgets

widget_control,Wlat_lev_base,iconify=0 ; de-iconify when drawing
wset,lat_lev_wdw

endif; endif device is X

end; end make_lat_lev() 
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; End make_lat_lev() event handler
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Begin Figure Latitude Level
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro lat_lev
@ibp_cmn.com
...
; Put dimensions in canonical order lon,lev,lat (in Fortran-IDL convention)
...
if (strlen(xttl_jd(fgr_idx)) le 0) then xttl_jd(fgr_idx)='Latitude !7u!5 (!E!12_!5!N)'
...
; Do zonal averaging
data=fltarr(lat_nbr,lev_nbr)
good_data_nbr=0
for lat_idx=0,lat_nbr-1 do begin
for lev_idx=0,lev_nbr-1 do begin
	good_idx=where(tmp_data(*,lev_idx,lat_idx) lt very_big_nbr,count)
	vld_cnt_pct=100.0*count/lon_nbr
	if vld_cnt_pct lt vld_avg_thr_pct then begin
		print,'WARNING: Only '+auto_sng(vld_cnt_pct,1)+'% of lons valid in zonal average of '+fld_nm+' at lat('+auto_sng(lat_idx,0)+') = '+auto_sng(lat(lat_idx),1)+' degrees.'
	endif
	if count ne 0 then begin
		good_data=tmp_data(good_idx,lev_idx,lat_idx) 
		data(lat_idx,lev_idx)=total(good_data)/count
		good_data_nbr=good_data_nbr+count
	endif else begin
		data(lat_idx,lev_idx)=1.0e36
	endelse
endfor; end loop over lev
endfor; end loop over lat

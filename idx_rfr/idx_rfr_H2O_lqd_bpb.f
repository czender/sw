c     $Id$
c     COMPLEX FUNCTION REFWAT( WL, TPT)
      subroutine reflqd( WL, TPT,idx_rfr_real,idx_rfr_img)
c     Author: Bruce P. Briegleb, NCAR
c     Modifications: Charlie Zender, NCAR
...
c                   TPT = temperature (K) -- for WL.GT.10.0 only
...
c     &    CALL ERRMSG( 'REFWAT--wavelength out of table range',.TRUE.)
     &    write (6,'(a)') 'REFWAT--wavelength out of table range'
...
c                                                  ** use Ray model
         REFWAT = RAYWAT( WL, TPT)

      END IF

c++csz
      idx_rfr_real=real(refwat)
      idx_rfr_img=aimag(refwat)
c--csz

      END

      COMPLEX FUNCTION RAYWAT( WL, TPT)
...
c     ..
      ...
      IF( WL.LT.WLMIN .OR. WL.GT.WLMAX )
c     &    CALL ERRMSG( 'RAYWAT--wavelength out of table range',.TRUE.)
     &     write (6,'(a)') 'REFWAT--wavelength out of table range'

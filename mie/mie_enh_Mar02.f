...
c------------------------------------------------------------
c    DOUBLE PRECISION 
c Program to calculate the enhancement coefficient g for
c absorption as a function of the size parameter
c x=k*a=2*pi*a/lambda for epsilon=m*m=const
c for a homogeneous distribution of impurities D=3
c Uses Bessel functions calculated by SPECFUN subroutines
c available from http://www.scd.ucar.edu/softlib/mathlib.html
c (in double precision). Quadruple precision not available.
c
c Input :
c     e - small constant. Summation over n stops when both
c         coefficients c_n^2 and d_n^2 become less than e
c     m - refractive index of water
c     xnfact - the ratio of the maximum order of Bessel functions
c         calculated to their argument. Should be greater than 1.
c         If xnfact is very large, Bessel functions will be calculated
c         inaccurately; if it is small, the summation in n will not 
c         converge. Currently set to 1.3, should work.
c     xmin, xmax, np - minimum (maximum) values of x for
c         which G is calculated, and number of points in x
c Output :
c     Output filename: enh.dat
c     File contains two columns of numbers x and G
c     NCALC - error indicator for subroutines RJBESL, RYBESL
c             (see subroutines' comments for explanation)
c------------------------------------------------------------
      ...
      write(*,*) 'Give xmin, xmax, np'
      read(*,*) xmin, xmax, np

      open(unit=70, file='enh.dat', status='unknown')
      
      do 100, ip=1,np

      x = xmin + float(ip-1)*(xmax-xmin)/float(np-1)
      x1 = m*x
      n = xnfact*x
 ...
 1    continue
      ...
      if(cc .lt. e) flag_cc = .true.
      if(dd .lt. e) flag_dd = .true.
      if(flag_cc .and. flag_dd) goto 3
      ccc = cc
      ddd = dd
      if(flag_cc) ccc = 0.
      if(flag_dd) ddd = 0.
      ...
      a1 = x1**2 * (dbj1(i))**2
      a2 = x1    * bj1(i) * dbj1(i)
      a3 = (x1**2 - xi*(xi+1.)) * (bj1(i))**2
      a4 = 0.5*x1*(a1 + a2 + a3)
      a5 = x1 * bj1(i) * (bj1(i) + x1*dbj1(i))

      s = s + (2.*xi+1.)*((ccc+ddd)*a4 + ddd*a5)
 2    continue
